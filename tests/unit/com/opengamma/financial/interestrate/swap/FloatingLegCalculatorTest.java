...
public class FloatingLegCalculatorTest {
  ...
  public void test() {
    ...
    assertEquals(CALCULATOR.getFloatLeg(FLAT_FORWARD_CURVE, FUNDING_CURVE, SWAP), 0, 0);
    assertEquals(CALCULATOR.getFloatLeg(FORWARD_CURVE, FLAT_FUNDING_CURVE, SWAP), 1. / DF_1 * DF_2, 1e-12);
    double expected = 0;
    for (int i = 1; i <= SWAP.getNumberOfFloatingPayments(); i++) {
      expected += Math.pow(DF_2, i) * (1. / DF_1 - 1);
    }
    assertEquals(CALCULATOR.getFloatLeg(FORWARD_CURVE, FUNDING_CURVE, SWAP), expected, 1e-12);
  }
}
