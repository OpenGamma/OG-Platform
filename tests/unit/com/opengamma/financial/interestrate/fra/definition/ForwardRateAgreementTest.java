...
public class ForwardRateAgreementTest {
  ...
  public void test() {
    ...
    final double end = 6;
    final ForwardRateAgreement fra = new ForwardRateAgreement(start, end);
    assertEquals(fra.getStartTime(), start, 0);
    assertEquals(fra.getEndTime(), end, 0);
    ForwardRateAgreement other = new ForwardRateAgreement(start, end);
    assertEquals(fra, other);
    assertEquals(fra.hashCode(), other.hashCode());
    other = new ForwardRateAgreement(start, end + 1);
    assertFalse(other.equals(fra));
    other = new ForwardRateAgreement(start + 1, end);
    assertFalse(other.equals(fra));
  }
}
