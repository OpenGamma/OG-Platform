...
public class YieldCurveBootStrapTest {
  ...
  static {
    ...
    SINGLE_CURVE_FINDER = new MultipleYieldCurveFinderFunction(SINGLE_CURVE_INSTRUMENTS, SWAP_VALUES, name_extrapolator_map, null);
    ...
    name_extrapolator_map.put(LIBOR_CURVE_NAME, fnInterpolator);
    SINGLE_CURVE_JACOBIAN = new MultipleYieldCurveFinderJacobian(SINGLE_CURVE_INSTRUMENTS, name_extrapolator_map, null);

    name_extrapolator_map = new LinkedHashMap<String, FixedNodeInterpolator1D>();
    fnInterpolator = new FixedNodeInterpolator1D(TIME_GRID, EXTRAPOLATOR_WITH_FD_SENSITIVITY);
    name_extrapolator_map.put(LIBOR_CURVE_NAME, fnInterpolator);
    SINGLE_CURVE_JACOBIAN_FD = new MultipleYieldCurveFinderJacobian(SINGLE_CURVE_INSTRUMENTS, name_extrapolator_map, null);
    ...
    name_extrapolator_map.put(LIBOR_CURVE_NAME, fnInterpolator);
    DOUBLE_CURVE_FINDER = new MultipleYieldCurveFinderFunction(DOUBLE_CURVE_INSTRUMENTS, SWAP_VALUES, name_extrapolator_map, null);
  ...
  private void doHotSpot(final VectorRootFinder rootFinder, final String name, final Function1D<DoubleMatrix1D, DoubleMatrix1D> functor, Boolean doubleCurveTest) {
  ...
  private void doTest(final VectorRootFinder rootFinder, final Function1D<DoubleMatrix1D, DoubleMatrix1D> functor, Boolean doubleCurveTest) {
    if (doubleCurveTest) {
  ...
  private void doTestForSingleCurve(final VectorRootFinder rootFinder, final Function1D<DoubleMatrix1D, DoubleMatrix1D> functor) {
    ...
    for (int i = 0; i < SWAP_VALUES.length; i++) {
      assertEquals(SWAP_VALUES[i], SWAP_RATE_CALCULATOR.getRate(SINGLE_CURVE_INSTRUMENTS.get(i), bundle), EPS);
    }
  }
  ...
  private void doTestForDoubleCurve(final VectorRootFinder rootFinder, final Function1D<DoubleMatrix1D, DoubleMatrix1D> functor) {
    ...
    double[] fundingYields = Arrays.copyOfRange(yieldCurveNodes.getData(), 0, FUNDING_CURVE_TIMES.length);
    double[] liborYields = Arrays.copyOfRange(yieldCurveNodes.getData(), FUNDING_CURVE_TIMES.length, yieldCurveNodes.getNumberOfElements());
    ...
    for (int i = 0; i < SWAP_VALUES.length; i++) {
      assertEquals(SWAP_VALUES[i], SWAP_RATE_CALCULATOR.getRate(DOUBLE_CURVE_INSTRUMENTS.get(i), bundle), EPS);
    }
  }
  ...
  public void testTickingSwapRates() {
    ...
    for (int t = 0; t < 100; t++) {
      ...
      final Function1D<DoubleMatrix1D, DoubleMatrix1D> functor = new MultipleYieldCurveFinderFunction(SINGLE_CURVE_INSTRUMENTS, swapRates, name_extrapolator_map, null);
      ...
      for (int i = 0; i < swapRates.length; i++) {
        assertEquals(swapRates[i], SWAP_RATE_CALCULATOR.getRate(SINGLE_CURVE_INSTRUMENTS.get(i), bundle), EPS);
      }
    }
  }
  ...
  public void testForwardCurveOnly() {
    ...
    final Function1D<DoubleMatrix1D, DoubleMatrix1D> functor = new MultipleYieldCurveFinderFunction(DOUBLE_CURVE_INSTRUMENTS, SWAP_VALUES, unknownCurves, knownCurves);

    unknownCurves = new LinkedHashMap<String, FixedNodeInterpolator1D>();
    fnInterpolator = new FixedNodeInterpolator1D(TIME_GRID, EXTRAPOLATOR_WITH_SENSITIVITY);
    unknownCurves.put(LIBOR_CURVE_NAME, fnInterpolator);
    final JacobianCalculator jacCal = new MultipleYieldCurveFinderJacobian(DOUBLE_CURVE_INSTRUMENTS, unknownCurves, knownCurves);
    ...
    for (int i = 0; i < SWAP_VALUES.length; i++) {
      assertEquals(SWAP_VALUES[i], SWAP_RATE_CALCULATOR.getRate(DOUBLE_CURVE_INSTRUMENTS.get(i), knownCurves), EPS);
    }
  }
