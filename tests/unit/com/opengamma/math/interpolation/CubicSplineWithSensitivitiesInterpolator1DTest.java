...
public class CubicSplineWithSensitivitiesInterpolator1DTest {
  ...
  public void testSensitivities() {
    ...
    for (int i = 0; i < 100; i++) {
      final double t = tmax * RANDOM.nextDouble();
      // double t = 0.25;
      final double[] sensitivity = INTERPOLATOR2.interpolate(DATA2, t).getSensitivities();
  ...
  public void testYieldCurve() {
    ...
    final Interpolator1DCubicSplineWithSensitivitiesDataBundle data = (Interpolator1DCubicSplineWithSensitivitiesDataBundle) Interpolator1DDataBundleFactory.fromSortedArrays(fwdTimes, rates,
        INTERPOLATOR2);
    final double[] sensitivity2 = INTERPOLATOR2.interpolate(data, 0.25).getSensitivities();
    final double[] sensitivity3 = INTERPOLATOR3.interpolate(data, 0.25).getSensitivities();
  ...
  private <T extends Interpolator1DDataBundle> double getSensitivity(final T model, final Interpolator1D<T, ? extends InterpolationResult> interpolator, final double t, final int node) {
    ...
    yUp[node] += EPS;
    yDown[node] -= EPS;
    final T modelUp = (T) Interpolator1DDataBundleFactory.fromSortedArrays(x, yUp, interpolator);
    final T modelDown = (T) Interpolator1DDataBundleFactory.fromSortedArrays(x, yDown, interpolator);
