...
public class RepositoryFactoryTest {
  public static class MockEmptyFunction extends AbstractFunction implements FunctionInvoker {
    @Override
  ...
  public void emptyConfiguration() {
    ...
    // This line is only necessary due to a bug in Fudge-Proto
    // http://jira.fudgemsg.org/browse/PRO-26
    configuration.setFunctions(new LinkedList<FunctionConfiguration>());
  ...
  public void singleConfigurationNoArgs() {
    ...
    FunctionInvoker invoker = repo.getInvoker(definition.getUniqueIdentifier());
  ...
  public void twoConfigurationsWithArgs() {
    ...
    configuration.addFunctions(new ParameterizedFunctionConfiguration(MockMultiArgumentFunctionArrayForm.class.getName(), MockMultiArgumentFunctionArrayForm.class.getName(), Lists.newArrayList("foo1", "foo2")));
    configuration.addFunctions(new ParameterizedFunctionConfiguration(MockMultiArgumentFunctionIndividualParameterForm.class.getName(), MockMultiArgumentFunctionIndividualParameterForm.class.getName(), Lists.newArrayList("bar1", "bar2")));
    InMemoryFunctionRepository repo = RepositoryFactory.constructRepository(configuration);
    assertNotNull(repo);
