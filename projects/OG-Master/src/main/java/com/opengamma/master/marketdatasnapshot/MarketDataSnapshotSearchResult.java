...
public class MarketDataSnapshotSearchResult extends AbstractSearchResult<MarketDataSnapshotDocument> {
   ...
   * Thus, this method implies an assumption about uniqueness of the queried snapshot.
   *
   * @param  <T> the required type of snapshot
   * @param  type the required type of snapshot, not null
   * @return the matching snapshot, not null
   * @throws IllegalStateException if no matching snapshot was found
   */
  public <T extends NamedSnapshot> T getSingleNamedSnapshot(Class<T> type) {
    if (getDocuments().size() != 1) {
      throw new OpenGammaRuntimeException("Expecting zero or single resulting match, and was " + getDocuments().size());
    } else {
      return getDocuments().get(0).getNamedSnapshot(type);
    }
  }
