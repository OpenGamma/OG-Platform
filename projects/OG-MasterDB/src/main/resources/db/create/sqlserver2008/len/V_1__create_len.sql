...
--     START WITH 1000 INCREMENT BY 1 NO CYCLE;
CREATE TABLE len_doc_seq (
  SeqID INT identity(1000,1) PRIMARY KEY,
  SeqVal VARCHAR(1)
)
CREATE TABLE len_idkey_seq (
  SeqID INT identity(1000,1) PRIMARY KEY,
  SeqVal VARCHAR(1)
)
CREATE TABLE len_attr_seq (
  SeqID INT identity(1000,1) PRIMARY KEY,
  SeqVal VARCHAR(1)
)
CREATE TABLE len_prop_seq (
  SeqID INT identity(1000,1) PRIMARY KEY,
  SeqVal VARCHAR(1)
)
...
CREATE INDEX ix_len_doc_name_type ON len_document(uname, main_type, sub_type, ver_from_instant, corr_from_instant, ver_to_instant, corr_to_instant);
CREATE INDEX ix_len_doc_sub_type ON len_document(usub_type, ver_from_instant, corr_from_instant, ver_to_instant, corr_to_instant);
