...
import com.opengamma.masterdb.historicaldata.DbHistoricalDataMaster;
...
public class DbHistoricalDataMasterTest extends DBTest {
  ...
  /** Logger. */
  private static final Logger s_logger = LoggerFactory.getLogger(DbHistoricalDataMasterTest.class);
  ...
  private DbHistoricalDataMaster _tsMaster;
  ...
  public DbHistoricalDataMasterTest(String databaseType, String databaseVersion) {
  ...
  public void searchByIdentifierBundle() throws Exception {
    ...
    HistoricalDataDocument tsDoc = getRandomTimeSeriesDocument(expectedTS);
  ...
  private HistoricalDataSearchResult search(
  ...
  public void searchByFieldProviderSource() throws Exception {
    ...
    HistoricalDataDocument tsDoc = getRandomTimeSeriesDocument(tsList);
      ...
      for (String dataSource : DATA_SOURCES) {
        for (String dataProvider : DATA_PROVIDERS) {
          for (String datafield : DATA_FIELDS) {
            ...
            HistoricalDataDocument tsDocument = createTimeSeries(datafield, dataProvider, dataSource, IdentifierBundleWithDates.of(identifiers), timeSeries);
            assertNotNull(tsDocument);
            assertNotNull(tsDocument.getUniqueId());
  ...
  protected HistoricalDataDocument createTimeSeries(final String datafield, final String dataProvider, final String dataSource, final IdentifierBundleWithDates identifiers, final LocalDateDoubleTimeSeries timeSeries) {
    HistoricalDataDocument tsDocument = new HistoricalDataDocument();
  ...
  public void addMultipleWithDateNulls() throws Exception {
    ...
    HistoricalDataDocument tsDoc = createTimeSeries("PX_LAST", "CMPL", "BBG", bundle, ts);
    assertNotNull(tsDoc);
    HistoricalDataDocument loadedTs = _tsMaster.get(tsDoc.getUniqueId());
   ...
   * @param tsDocument
   */
  private void setIdentifiers(IdentifierBundle identifiers, HistoricalDataDocument tsDocument) {
  ...
  private HistoricalDataDocument getRandomTimeSeriesDocument(List<HistoricalDataDocument> tsList) {
  ...
  public void removeThenAddTimeseries() throws Exception {
    ...
    HistoricalDataDocument currentTSDoc = createTimeSeries(tsDoc.getDataField(), tsDoc.getDataProvider(), tsDoc.getDataSource(), tsDoc.getIdentifiers(), currentTS);
    assertNotNull(currentTSDoc);
    assertNotNull(currentTSDoc.getUniqueId());
    
    HistoricalDataDocument actualDoc = _tsMaster.get(currentTSDoc.getUniqueId());
    assertNotNull(actualDoc);
  ...
  public void getUnknownUID() throws Exception {
    addAndTestTimeSeries();
    try {
      _tsMaster.get(UniqueIdentifier.of(DbHistoricalDataMaster.IDENTIFIER_SCHEME_DEFAULT, String.valueOf(Long.MIN_VALUE)));
      fail();
    } catch(DataNotFoundException ex) {
      //do nothing
    }
  ...
  public void searchMetaData() throws Exception {
    ...
    for (HistoricalDataDocument testDoc : documents) {
  ...
  public void getTimeSeriesSnapShot() throws Exception {
    ...
    for (Entry<ZonedDateTime, LocalDateDoubleTimeSeries> entry : timeStampTSMap.entrySet()) {
      ...
      HistoricalDataDocument snapshotDoc = getTimeSeriesSnapShot(identifiers, timeStamp);
    ...
    HistoricalDataDocument snapshotDoc = getTimeSeriesSnapShot(identifiers, beforeDelta);
  ...
  private HistoricalDataDocument getTimeSeriesSnapShot(IdentifierBundle identifiers, ZonedDateTime timeStamp) {
    HistoricalDataSearchHistoricRequest searchHistoricRequest = new HistoricalDataSearchHistoricRequest();
    ...
    HistoricalDataSearchHistoricResult searchHistoric = _tsMaster.searchHistoric(searchHistoricRequest);
    assertNotNull(searchHistoric);
    List<HistoricalDataDocument> documents = searchHistoric.getDocuments();
  ...
  public void identifiersWithDates() throws Exception {
    ...
    HistoricalDataDocument tsDocument = createTimeSeries(CLOSE_DATA_FIELD, CMPL_DATA_PROVIDER, BBG_DATA_SOURCE, IdentifierBundleWithDates.of(new IdentifierWithDates[]{edu0, edu10, eduBuid}), timeSeries);
    assertNotNull(tsDocument);
    assertNotNull(tsDocument.getUniqueId());
  ...
  public void identifiersWithDatesIntersectLowerBound() throws Exception {
    ...
    HistoricalDataDocument tsDocument = createTimeSeries(CLOSE_DATA_FIELD, CMPL_DATA_PROVIDER, BBG_DATA_SOURCE, bundle, getEmptyTimeSeries());
    assertNotNull(tsDocument);
    assertNotNull(tsDocument.getUniqueId());
  ...
  //-------------------------------------------------------------------------
  protected DbHistoricalDataMaster getTimeSeriesMaster(Map<String, String> namedSQLMap) {
    return new LocalDateDbHistoricalDataMaster(
        getDbSource(), 
        namedSQLMap,
        false);
  }
