...
private:
	...
	static TCHAR *FindServiceExecutable (const TCHAR *pszServiceName) {
		...
		HRESULT hr;
		HKEY hkey;
		if ((hr = RegOpenKeyEx (HKEY_LOCAL_MACHINE, TEXT ("CurrentControlSet\\service"), 0, KEY_READ, &hkey)) == ERROR_SUCCESS) {
			...
			LOGWARN (TEXT ("Couldn't open HKLM\\CurrentControlSet\\service registry key, error ") << hr);
		}
#endif /* ifdef _WIN32 */
