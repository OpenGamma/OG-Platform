...
/* package */final class StructuredMarketDataSnapshotUtil {
  ...
  private static final CompositeStringUtil s_surfaceName = new CompositeStringUtil(3, false);
  ...
  public static VolatilitySurfaceKey toVolatilitySurfaceKey(final String name) {
    ...
    return new VolatilitySurfaceKey(UniqueId.parse(parsed[0]), parsed[1], parsed[2]);
  }

  public static String fromVolatilitySurfaceKey(final VolatilitySurfaceKey key) {
    return s_surfaceName.create(key.getTarget().toString(), key.getName(), key.getInstrumentType());
  }
