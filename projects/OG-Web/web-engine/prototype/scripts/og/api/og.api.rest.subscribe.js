    ...
    obj: function () {
        ...
        return subscribe = api.handshake.get.partial({handler: function (result) {
            ...
            (fire_updates = function (reset, result) {
                ...
                api.registrations = api.registrations // throw out stale registrations
                    .filter(function (reg) {return !common.request_expired(reg, current);});
                if (reset) return api.registrations = api.registrations // fire all updates if connection is reset
                ...
                result.data.updates.filter(function (update) {
                    ...
                    for (lcv = 0; lcv < len; lcv += 1)
                        if ((reg = api.registrations[lcv]) && reg.url === update)
                            (api.registrations[lcv] = null), reg.update(reg);
            ...
            (listen = function () {
                api.updates.get({handler: function (result) {
                    ...
                    if (!result.data || !result.data.updates.length) return setTimeout(listen, common.INSTANT);
