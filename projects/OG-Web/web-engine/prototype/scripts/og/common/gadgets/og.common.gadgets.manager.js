...
$.register_module({
    ...
    obj: function () {
        var manager, collector, gadgets = [], interval = 5 * 60 * 1000, layout,
            resize = function () {
                return layout && manager.clean().forEach(function (gadget) {gadget.resize.apply(gadget.context, []);});
            };
        ...
        return manager = {
            clean: function () {
                return gadgets = gadgets.filter(function (gadget) {
                    ...
                    try {return gadget.alive.apply(gadget.context, []);}
                    catch (error) {return og.dev.warn(error), false;}
                });
            },
