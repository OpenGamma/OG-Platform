...
$.register_module({
    ...
    obj: function () {
        var module = this, loading_template, formatters, char_width = 9;
        var cell_value = function (v) {return {v: v + ''};};
        var meta = function (dataman, rows, cols, fixed_width, first_fixed) {
            ...
            meta = {
                ...
                columns: {
                    fixed: [{columns: [{type: 'STRING', header: first_fixed ? cols[0] : '', width: fixed_width}]}],
                    scroll: [{
                        columns: cols.slice(first_fixed ? 1 : 0)
                            .map(function (name) {return {type: 'STRING', header: name};})
                    }],
                    total: first_fixed ? cols.length : cols.length + 1
                }
            };
