    ...
    obj: function () {
        ...
        var constructor = function (config) {
            ...
            var grid_setup = function () {
                if (!view_id) return;
                return depgraph ?
                    api.grid.structure.get({view_id: view_id, update: initialize}).pipe(function (result) {
                        if (result.error || !result.data[SETS].length) return result; // goes to grid_handler
                        if (graph_id) return api.grid.depgraphs.structure
                            .get({view_id: view_id, graph_id: graph_id, update: initialize});
                        return api.grid.depgraphs.put({row: config.row, col: config.col, view_id: view_id})
                            .pipe(function (result) {
                                return api.grid.depgraphs.structure
                                    .get({view_id: view_id, graph_id: (graph_id = result.meta.id), update: initialize});
                            })
                    })
                    : api.grid.structure.get({view_id: view_id, update: initialize});
            };
            ...
            var view_handler = function (result) {
                ...
                return (view_id = result.meta.id), grid_setup();
            };
