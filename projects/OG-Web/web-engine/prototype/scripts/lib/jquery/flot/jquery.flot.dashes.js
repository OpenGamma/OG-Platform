(function($) {
    $.plot.plugins.push({
        init: init, name: 'dashes', version: '1.0',
        options: {
            series: {
                dashes: {
    ...
    function init(plot) {
        plot.hooks.drawSeries.push(function(plot, canvascontext, series) {
            ...
            function plotter() {
                var points = series.datapoints.points, ps = series.datapoints.pointsize, axisx = series.xaxis,
                    ...
                    for (var i = ps; i < points.length; i += ps) {
                        ...
                        while(true) {
                            var segmentLength = remainder > 0 ? remainder : drawing ? dashLength : spaceLength;
