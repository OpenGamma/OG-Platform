<#escape x as x?html>
{
    ...
    "trades": [
        <#list position.trades as trade>{
            ...
            "date_time": "${trade.tradeDate}<#if trade.tradeTime?has_content> @ ${trade.tradeTime.toString(timeFormatter)} (${trade.premiumTime.toString(offsetFormatter)})</#if>",
            "premium": "${trade.premium}<#if trade.premiumCurrency?has_content> ${trade.premiumCurrency}</#if>",
            "premium_date_time": "${trade.premiumDate}<#if trade.premiumTime?has_content> @ ${trade.premiumTime.toString(timeFormatter)} (${trade.tradeTime.toString(offsetFormatter)})</#if>",
