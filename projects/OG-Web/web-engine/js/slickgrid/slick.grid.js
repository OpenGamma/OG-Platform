...
(function($) {
    ...
    function SlickGrid($container,data,columns,options) {
        ...
        /* begin OpenGamma */
        /**
         * Synchronously runs the post-processing on a collection of rows without completely re-rendering them.
         * 
         * @param rows
         */
        function reprocessRows(rows) {
          if (!rows || !rows.length) {
            // Nothing to process
            return;
          }
        ...
        function reprocessRow(row) {
          var rowNode = rowsCache[row];
          if (!rowNode || row>=gridDataGetLength()) { return; }
          var d = gridDataGetItem(row), cellNodes = rowNode.childNodes;
          for (var i=0, j=0, l=columns.length; i<l; ++i) {
              var m = columns[i];
              if (m.asyncPostRender) {
                m.asyncPostRender(cellNodes[j], row, d, m);
              }
              ++j;
          }
        }
        ...
        function handleMouseUp(e) {
            if (click_target !== e.target) {
                // this will be lost as a click - we REALLY should check it's actually a click here at some point.
                handleClick(e);
            }
        }
        /* end OpenGamma */
        ...
        $.extend(this, {
            ...
            /* begin OpenGamma */
            "reprocessRows": reprocessRows,
            "reprocessAllRows": reprocessAllRows,
            /* end OpenGamma */
