 ...
 */
public abstract class AbstractEHCachingSource<S extends Source<T> & ChangeProvider, T extends UniqueIdentifiable> implements Source<T>, ChangeProvider {

  /** Logger. */
  private static final Logger s_logger = LoggerFactory.getLogger(AbstractEHCachingSource.class);

  /** The oid cache key. */
  final String OID_CACHE = getClass().getName() + "-oid-cache";
  /** The uid cache key. */
  final String UID_CACHE = getClass().getName() + "-uid-cache";
   ...
   */
  private final ConcurrentMap<UniqueId, T> _frontCache = new MapMaker().weakValues().makeMap();

  /**
   * EHCache doesn't like being hammered repeatedly for the same objects. Also, if the window of objects being requested is bigger than the in memory window then new objects get created as the on-disk
   * values get deserialized. The solution is to maintain a soft referenced buffer so that all the while the objects we have previously returned are in use we won't requery EHCache for them.
   */
  protected final Map2<Object, VersionCorrection, T> _frontCache2 = new SoftValueHashMap2<Object, VersionCorrection, T>();

  private final HashDeepMap2<ObjectId, UniqueId, T> _frontCache3 = new HashDeepMap2<ObjectId, UniqueId, T>() {
   ...
   * The oid cache.
   */
  protected final Cache _oidCache;
  /**
   * The uid cache.
   */
  protected final Cache _uidCache;
  ...
  public AbstractEHCachingSource(final S underlying, final CacheManager cacheManager) {
    ...
    EHCacheUtils.addCache(cacheManager, OID_CACHE);
    EHCacheUtils.addCache(cacheManager, UID_CACHE);
    _oidCache = EHCacheUtils.getCacheFromManager(cacheManager, OID_CACHE);
    _uidCache = EHCacheUtils.getCacheFromManager(cacheManager, UID_CACHE);
  ...
  protected Map<VersionCorrection, T> getObjectIdCacheEntry(final ObjectId objectId) {
    final Element e = _oidCache.get(objectId);
    if (e != null) {
      if (e.getObjectValue() instanceof Map<?, ?>) {
        return (Map<VersionCorrection, T>) e.getObjectValue();
      }
    }
    return null;
  }
  ...
  @Override
  public T get(final ObjectId objectId, final VersionCorrection versionCorrection) {
    ArgumentChecker.notNull(objectId, "objectId");
    ArgumentChecker.notNull(versionCorrection, "versionCorrection");
    T result = _frontCache2.get(objectId, versionCorrection);
    if (result != null) {
      return result;
    }
    Map<VersionCorrection, T> items = getObjectIdCacheEntry(objectId);
    if (items != null) {
      result = items.get(versionCorrection);
      if (result != null) {
        final T existing = _frontCache2.putIfAbsent(objectId, versionCorrection, result);
        if (existing != null) {
          return existing;
        }
      }
    ...
    if (result == null) {
      ...
      final Map<VersionCorrection, T> newitems = new HashMap<VersionCorrection, T>();
      synchronized (this) {
  ...
  public void shutdown() {
    ...
    _manager.removeCache(UID_CACHE);
    _manager.removeCache(OID_CACHE);
  }
  ...
  protected void cacheItems(Collection<T> items) {
    for (T security : items) {
