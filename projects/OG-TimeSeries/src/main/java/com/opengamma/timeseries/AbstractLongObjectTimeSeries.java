 ...
 */
public abstract class AbstractLongObjectTimeSeries<DATE_TYPE, T>
    extends AbstractFastBackedObjectTimeSeries<DATE_TYPE, T> {

  /** Serialization version. */
  private static final long serialVersionUID = 8169275066251744746L;
  ...
  public boolean containsTime(DATE_TYPE dateTime) {
    return getFastSeries().containsTime(_converter.convertToLong(dateTime));
  }
  ...
  @Override
  public T getValueAtIndex(final int index) {
  ...
  protected class IteratorAdapter implements Iterator<Entry<DATE_TYPE, T>> {
    ...
    public Entry<DATE_TYPE, T> next() {
      ...
      return _converter.makeMapEntry(_converter.convertFromLong(next.getKey()), next.getValue());
    }
