...
import com.opengamma.util.timeseries.TimeSeriesUtils;
...
public class FastMapIntDoubleTimeSeries extends AbstractFastMutableIntDoubleTimeSeries {
  ...
  public FastMapIntDoubleTimeSeries(final DateTimeNumericEncoding encoding, final int[] times, final double[] values) {
    ...
    if (times.length != values.length) {
      throw new IllegalArgumentException("times and values arrays must be the same length");
    }
  ...
  public FastMapIntDoubleTimeSeries(final DateTimeNumericEncoding encoding, final List<Integer> times,
      final List<Double> values) {
    super(encoding);
    if (times.size() != values.size()) {
      throw new IllegalArgumentException("times and values lists must be the same length");
    }
  ...
  public ObjectIterator<Int2DoubleMap.Entry> iteratorFast() {
    return (ObjectIterator<Int2DoubleMap.Entry>) (ObjectIterator<? extends Entry<Integer, Double>>) _map.entrySet()
        .iterator();
  }
  ...
  public FastIntDoubleTimeSeries headFast(final int numItems) {
    if (_map.size() < numItems) {
      throw new IllegalStateException("cannot get head " + numItems +
    ...
    if (getClass() != obj.getClass()) {
      if (obj instanceof FastIntDoubleTimeSeries) {
        ...
        while (otherIterator.hasNext()) {
          ...
          if (!TimeSeriesUtils.closeEquals(otherNext.getDoubleValue(), myNext.getDoubleValue())) {
            return false;
          }
