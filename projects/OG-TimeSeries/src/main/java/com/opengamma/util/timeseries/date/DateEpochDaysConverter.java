...
/**
 * Converts dates to numbers.
 */
public class DateEpochDaysConverter implements DateTimeConverter<Date> {

  /** Logger. */
  private static final Logger s_logger = Logger.getLogger(DateEpochDaysConverter.class.getName());
  ...
  public DateEpochDaysConverter(final TimeZone timeZone) {
    _timeZone = timeZone;
  }
  ...
  @Override
  public <T> ObjectTimeSeries<Date, T> convertFromInt(ObjectTimeSeries<Date, T> templateTS, FastIntObjectTimeSeries<T> pidts) {
    final Calendar cal = _calendar.get();
    final Date[] dates = new Date[pidts.size()];
    ...
    while (iterator.hasNext()) {
      ...
      if (s_logger.isLoggable(Level.FINE)) {
        if (entry.getKey().getTime() % MILLIS_PER_DAY != 0) {
          s_logger.warning("losing precision on conversion of dates to ints (epoch days)");
        }
      }
      dates[i] = epochDays;
      values[i] = entry.getValue();
  ...
  public int convertToInt(final Date dateTime) {
    if (s_logger.isLoggable(Level.FINE)) {
      if (dateTime.getTime() % MILLIS_PER_DAY != 0) {
        s_logger.warning("losing precision on conversion of date to int (epoch days)");
      }
    }
  ...
  public <T> FastLongObjectTimeSeries<T> convertToLong(
      FastLongObjectTimeSeries<T> templateTS, ObjectTimeSeries<Date, T> dts) {
    final Iterator<Entry<Date, T>> iterator = dts.iterator();
    final long[] dateTimes = new long[dts.size()];
    final T[] values = (T[]) new Object[dts.size()];
    int i = 0;
  ...
  @Override
  public <T> Map.Entry<Date, T> makeMapEntry(Date dateTime, T value) {
    return new SimpleImmutableEntry<Date, T>(dateTime, value);
  }
