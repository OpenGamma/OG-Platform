...
public class MapYearOffsetObjectTimeSeries<T> extends MutableYearOffsetObjectTimeSeries.Long<T> {
  ...
  public MapYearOffsetObjectTimeSeries(final ZonedDateTime zeroDate, final Double[] dates, final T[] values) {
    super(new YearOffsetEpochMillisConverter(zeroDate), new FastMapLongObjectTimeSeries<T>(DateTimeNumericEncoding.TIME_EPOCH_MILLIS, 
          new YearOffsetEpochMillisConverter(zeroDate).convertToLong(dates), values));
  }
  ...
  public MapYearOffsetObjectTimeSeries(final YearOffsetObjectTimeSeries<T> dts) {
    super(new YearOffsetEpochMillisConverter(((YearOffsetEpochMillisConverter) dts.getConverter()).getZonedOffset()), 
        (FastMutableLongObjectTimeSeries<T>) new YearOffsetEpochMillisConverter(((YearOffsetEpochMillisConverter) dts.getConverter()).getZonedOffset())
          .convertToLong(new FastMapLongObjectTimeSeries<T>(DateTimeNumericEncoding.TIME_EPOCH_MILLIS), dts));
  }

  @SuppressWarnings({"unchecked", "rawtypes" })
  public MapYearOffsetObjectTimeSeries(final ZonedDateTime zeroDate, final YearOffsetObjectTimeSeries<T> dts) {
    super(new YearOffsetEpochMillisConverter(zeroDate), (FastMutableLongObjectTimeSeries<T>) new YearOffsetEpochMillisConverter(zeroDate)
      .convertToLong(new FastMapLongObjectTimeSeries(DateTimeNumericEncoding.TIME_EPOCH_MILLIS), dts));
  }
