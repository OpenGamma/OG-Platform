...
public final class Failure implements ImmutableBean {
  ...
  /**
   * Stack traces can take up a lot of memory if there are a large number of failure results in the cache.
   * They are often duplicated many times so interning them can save a significant amount of memory.
   */
  private static final Interner<String> s_stackTraceInterner = Interners.newWeakInterner();
  
  /**
   * An enormous number of Failure objects get constructed during Sesame
   * requirements gathering. This has significant performance impacts for
   * large portfolios with large amounts of market data requirements.
   * This guard stops generating them.
   */
  private static final ThreadLocal<Boolean> s_generateStackTrace = new ThreadLocal<Boolean>() {
    private final AtomicBoolean _generateStackTrace = new AtomicBoolean(true);

    @Override
    public Boolean get() {
      return _generateStackTrace.get();
    }

    @Override
    public void set(Boolean value) {
      _generateStackTrace.set(value.booleanValue());
    }
  ...
  Failure(FailureStatus status, String message, Exception cause) {
    ...
    if (s_generateStackTrace.get()) {
      _stackTrace = s_stackTraceInterner.intern(Throwables.getStackTraceAsString(cause));
    } else {
      _stackTrace = "N/A";
    }
  }
  ...
  /**
   * Use to turn off stack trace generation during Sesame results gathering.
   */
  public static void turnOffStackTraces() {
    s_generateStackTrace.set(false);
  }
  
  /**
   * Use to turn on stack trace generation after Sesame results gathering.
   */
  public static void turnOnStackTraces() {
    s_generateStackTrace.set(true);
  }
