...
public class ArraySQLDateObjectTimeSeries<T> extends SQLDateObjectTimeSeries.Integer<T> {
  ...
  public ArraySQLDateObjectTimeSeries(final TimeZone timeZone, final Date[] dates, final T[] values) {
    super(new SQLDateEpochDaysConverter(timeZone), new FastArrayIntObjectTimeSeries<T>(DateTimeNumericEncoding.DATE_EPOCH_DAYS, new SQLDateEpochDaysConverter(timeZone).convertToInt(dates),
        values));
  }
  ...
  public ArraySQLDateObjectTimeSeries(final TimeZone timeZone, final ObjectTimeSeries<Date, T> dts) {
    super(new SQLDateEpochDaysConverter(timeZone), new SQLDateEpochDaysConverter(timeZone).convertToInt(new FastListIntObjectTimeSeries<T>(DateTimeNumericEncoding.DATE_EPOCH_DAYS), dts));
  }
