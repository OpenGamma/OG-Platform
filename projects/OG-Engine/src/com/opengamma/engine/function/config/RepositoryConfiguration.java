...
public class RepositoryConfiguration implements java.io.Serializable {
  ...
  protected RepositoryConfiguration (final org.fudgemsg.mapping.FudgeDeserializationContext fudgeContext, final org.fudgemsg.FudgeMsg fudgeMsg) {
    ...
    if (fudgeFields.size () > 0)  {
      ...
      for (org.fudgemsg.FudgeField fudge2 : fudgeFields) {
        try {
          ...
          fudge3 = com.opengamma.engine.function.config.FunctionConfiguration.fromFudgeMsg (fudgeContext, fudgeMsg.getFieldValue (org.fudgemsg.FudgeMsg.class, fudge2));
          fudge1.add (fudge3);
        }
  ...
  public static RepositoryConfiguration fromFudgeMsg (final org.fudgemsg.mapping.FudgeDeserializationContext fudgeContext, final org.fudgemsg.FudgeMsg fudgeMsg) {
    final java.util.List<org.fudgemsg.FudgeField> types = fudgeMsg.getAllByOrdinal (0);
    for (org.fudgemsg.FudgeField field : types) {
      final String className = (String)field.getValue ();
      if ("com.opengamma.engine.function.config.RepositoryConfiguration".equals (className)) break;
      try {
        return (com.opengamma.engine.function.config.RepositoryConfiguration)Class.forName (className).getDeclaredMethod ("fromFudgeMsg", org.fudgemsg.mapping.FudgeDeserializationContext.class, org.fudgemsg.FudgeMsg.class).invoke (null, fudgeContext, fudgeMsg);
      }
      catch (Throwable t) {
        // no-action
      }
    }
    return new RepositoryConfiguration (fudgeContext, fudgeMsg);
  }
