...
public final class PLAT3249 {
  ...
  private DependencyNode getOrCreateCopy(DependencyNode node) {
    ...
    if (newNode == null) {
      newNode = new DependencyNode(node.getComputationTarget());
      newNode.setFunction(node.getFunction());
      newNode.addOutputValues(node.getOutputValues());
      for (ValueSpecification terminalOutput : node.getTerminalOutputValues()) {
        newNode.addTerminalOutputValue(terminalOutput);
      }
      for (ValueSpecification input : node.getInputValues()) {
        newNode.addInputValue(input);
      }
