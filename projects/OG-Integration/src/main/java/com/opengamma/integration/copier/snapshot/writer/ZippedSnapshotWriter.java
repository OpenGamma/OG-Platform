...
/**
 * This class writes positions/securities to a zip file, using the zip file's directory structure to represent the snapshot
 * node structure.
 */
public class ZippedSnapshotWriter implements SnapshotWriter {

  private static final Logger s_logger = LoggerFactory.getLogger(ZippedSnapshotWriter.class);
  ...
  private SingleSheetMultiParserSnapshotWriter _currentWriter;
  private Map<String, SingleSheetMultiParserSnapshotWriter> _writerMap = new HashMap<String, SingleSheetMultiParserSnapshotWriter>();
  ...
  public ZippedSnapshotWriter(SheetFormat sheetFormat, OutputStream outputStream) {
  ...
  public ZippedSnapshotWriter(SheetFormat sheetFormat, OutputStream outputStream, boolean includeTrades) {
    this(sheetFormat, outputStream);
    _includeTrades = includeTrades;
  }

  public ZippedSnapshotWriter(String filename) {
  ...
  public ZippedSnapshotWriter(String filename, boolean includeTrades) {
    this(filename);
    _includeTrades = includeTrades;
  }
  ...
  public ObjectsPair<ManageablePosition, ManageableSecurity[]> writePosition(ManageablePosition position, ManageableSecurity[] securities) {
    ...
    identifyOrCreateSnapshotWriter(securities[0]);
  ...
  private void flushCurrentBuffers() {
    ...
    for (Entry<String, ByteArrayOutputStream> entry : _bufferMap.entrySet()) {
    ...
    _writerMap = new HashMap<String, SingleSheetMultiParserSnapshotWriter>();
  }

  private SnapshotWriter identifyOrCreateSnapshotWriter(ManageableSecurity security) {
    
    // Identify the correct snapshot writer for this security
    ...
    if (_currentWriter == null) {
      ...
      _currentWriter = new SingleSheetMultiParserSnapshotWriter(sheet, parserMap, _includeTrades);
  ...
  private void writeMetaData(String filename) {
    try {
      ...
      _zipFile.write("# Generated by OpenGamma zipped snapshot writer\n\n".getBytes());
