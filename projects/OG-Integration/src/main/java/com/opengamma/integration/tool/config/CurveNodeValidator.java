...
public final class CurveNodeValidator implements CurveNodeVisitor<Void> {
  ...
  /**
   * 
   * @param curveValidator 
   * @param conventionValidatorUtil
   * @param securitySource
   * @param validationNode
   * @param curveNodeIdMapper
   */
  public CurveNodeValidator(LocalDate curveDate, ConfigValidationUtils configValidationUtils, SecuritySource securitySource, ValidationNode validationNode, CurveNodeIdMapper curveNodeIdMapper) {
  ...
  public Void visitCalendarSwapNode(CalendarSwapNode node) {
    ...
    try {
      calendarNodeId = _curveNodeIdMapper.getCalendarSwapNodeId(_curveDate, node.getStartTenor(), node.getCalendarDateStartNumber(), node.getCalendarDateEndNumber());
    } catch (OpenGammaRuntimeException ogre) {
      calendarNodeId = null;
    }
    ValidationNode calendarSwapValidationNode;
    if (calendarNodeId == null) {
      if (!_configValidationUtils.holidayExists(node.getCalendarId())) {
      ...
      if (!_configValidationUtils.holidayExists(calendarNodeId)) {
        calendarSwapValidationNode = createInvalidCurveNodeValidationNode(node.getStartTenor(), CalendarSwapNode.class, _validationNode, 
