...
/**
 * Snapshot reader that reads multiple CSV files within a ZIP archive, identifies the correct parser class for each,
 * using the file name, and persists all loaded trades/entries using the specified snapshot writer. Folder structure
 * in the ZIP archive is replicated in the snapshot node structure.
 */
public class ZippedSnapshotReader implements SnapshotReader {

  private static final Logger s_logger = LoggerFactory.getLogger(ZippedSnapshotReader.class);
  ...
  private SnapshotReader _currentReader;
  private String[] _currentPath = new String[0];
  private boolean _ignoreVersion;

  @SuppressWarnings("unchecked")
  public ZippedSnapshotReader(String filename, boolean ignoreVersion) {
  ...
  public ObjectsPair<ManageablePosition, ManageableSecurity[]> readNext() {
    ...
    while (true) {      
      ...
      // More zip entries, try get another csv and return a row from it
        ...
        // Get the snapshot reader for this csv file
        _currentReader = getReader(_zipEntries.nextElement());            
      }
    }
  }

  private SnapshotReader getReader(ZipEntry entry) {
    ...
    if (!entry.isDirectory() && entry.getName().substring(entry.getName().lastIndexOf('.')).equalsIgnoreCase(SHEET_EXTENSION)) {
      try {
        ...
        // Create a simple snapshot reader for the current sheet
        return new SingleSheetSimpleSnapshotReader(sheet, parser);
