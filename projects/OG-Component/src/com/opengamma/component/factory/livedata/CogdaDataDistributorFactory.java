...
public abstract class CogdaDataDistributorFactory extends AbstractComponentFactory {
  private static final Logger s_logger = LoggerFactory.getLogger(CogdaDataDistributorFactory.class);
  ...
  protected LastKnownValueStoreProvider constructLastKnownValueStoreProvider() {
    if (getRedisServer() == null) {
      s_logger.info("No Redis Server specified in configuration so using map LKVStoreProvider.");
      ...
      lkvProvider.setServer(getRedisServer());
      lkvProvider.setWriteThrough(true);
      if (getRedisPort() == null) {
        s_logger.info("No Redis port provided. Defaulting to 6379");
        setRedisPort(6379);
        lkvProvider.setPort(getRedisPort());
      }
      if (getRedisPrefix() != null) {
        s_logger.info("Setting Redis key prefix to {}", getRedisPrefix());
        lkvProvider.setGlobalPrefix(getRedisPrefix());
      }
      return lkvProvider;
    }
  }
