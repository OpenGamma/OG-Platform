...
/**
 * Unit test for SecurityAndTradingExchangeExposureFunction.
 */
@Test(groups = TestGroup.UNIT)
public class SecurityAndTradingExchangeExposureFunctionTest {
  private static final ExposureFunction EXPOSURE_FUNCTION = new SecurityAndTradingExchangeExposureFunction();
  ...
  public void testEnergyFutureOptionSecurity() {
    ...
    List<ExternalId> ids = EXPOSURE_FUNCTION.getIds(trade);
    assertEquals(1, ids.size());
    assertEquals(ExternalId.of(ExposureFunction.SECURITY_IDENTIFIER, "COMMODITYFUTURE_OPTION_X"), ids.get(0));
  }
