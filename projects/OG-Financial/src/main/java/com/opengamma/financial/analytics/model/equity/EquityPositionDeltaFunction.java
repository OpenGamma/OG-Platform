 ...
 *
 */
public class EquityPositionDeltaFunction  extends AbstractFunction.NonCompiledInvoker {

  private String getValueRequirementName() {
    return ValueRequirementNames.POSITION_DELTA;
  }
  ...
  public Set<ComputedValue> execute(FunctionExecutionContext executionContext, FunctionInputs inputs, ComputationTarget target, Set<ValueRequirement> desiredValues) throws AsynchronousExecution {
    ...
    final ComputedValue result = new ComputedValue(valueSpecification, target.getPosition().getQuantity());
    return Sets.newHashSet(result);
  }
  
  @Override
  public boolean canApplyTo(FunctionCompilationContext context, ComputationTarget target) {
    if (target.getPosition().getSecurity() instanceof EquitySecurity) {
      return true;
    }
    return false;
  }
