...
public class CorrelationMatrixFunction extends AbstractFunction.NonCompiledInvoker {
  ...
  public static final String VALUE_NAME = "CorrelationMatrix";
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target) {
    return Collections.singleton(new ValueSpecification(VALUE_NAME, target.toSpecification(), ValueProperties.all()));
  }

  @Override
  public Set<ValueRequirement> getRequirements(final FunctionCompilationContext context, final ComputationTarget target, final ValueRequirement desiredValue) {
    return Collections.singleton(new ValueRequirement(SampledCovarianceMatrixFunction.VALUE_NAME, target.toSpecification(), desiredValue.getConstraints().withoutAny(ValuePropertyNames.FUNCTION)));
  }

  @Override
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target, final Map<ValueSpecification, ValueRequirement> inputs) {
    final ValueSpecification input = inputs.keySet().iterator().next();
    return Collections.singleton(new ValueSpecification(VALUE_NAME, target.toSpecification(), input.getProperties().copy().withoutAny(ValuePropertyNames.FUNCTION)
  ...
  public Set<ComputedValue> execute(final FunctionExecutionContext context, final FunctionInputs inputs, final ComputationTarget target, final Set<ValueRequirement> desiredValues) {
    ...
    final DoubleLabelledMatrix2D input = (DoubleLabelledMatrix2D) inputs.getValue(SampledCovarianceMatrixFunction.VALUE_NAME);
    ...
    return Collections.singleton(new ComputedValue(new ValueSpecification(VALUE_NAME, target.toSpecification(), desiredValue.getConstraints()), output));
  }
