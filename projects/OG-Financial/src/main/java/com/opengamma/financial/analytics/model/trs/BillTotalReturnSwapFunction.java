...
import com.opengamma.financial.analytics.conversion.BillTotalReturnSwapSecurityConverter;
 ...
 */
public abstract class BillTotalReturnSwapFunction extends DiscountingFunction {

  /**
   * @param valueRequirements The value requirement names, not null
   */
  public BillTotalReturnSwapFunction(final String... valueRequirements) {
  ...
  protected DefaultTradeConverter getTargetToDefinitionConverter(final FunctionCompilationContext context) {
    ...
    final FinancialSecurityVisitor<InstrumentDefinition<?>> securityConverter = new BillTotalReturnSwapSecurityConverter(conventionSource,
        holidaySource, regionSource, securitySource, legalEntitySource);
   ...
   */
  protected abstract class BillTotalReturnSwapCompiledFunction extends DiscountingCompiledFunction {
     ...
     */
    protected BillTotalReturnSwapCompiledFunction(final DefaultTradeConverter tradeToDefinitionConverter,
    ...
    public boolean canApplyTo(final FunctionCompilationContext context, final ComputationTarget target) {
      return target.getTrade().getSecurity() instanceof BillTotalReturnSwapSecurity;
    }
    ...
    protected Collection<ValueProperties.Builder> getResultProperties(final FunctionCompilationContext context, final ComputationTarget target) {
      ...
      if (isWithCurrency()) {
        properties.with(CURRENCY, getCurrencyOfResult((BillTotalReturnSwapSecurity) target.getTrade().getSecurity()));
      }
     ...
     */
    protected abstract String getCurrencyOfResult(BillTotalReturnSwapSecurity security);
  }
}
