...
public class EquityOptionBlackVommaFunction extends EquityOptionBlackFunction {
  ...
  protected Set<ComputedValue> computeValues(final InstrumentDerivative derivative, final StaticReplicationDataBundle market, final FunctionInputs inputs,
    ...
    if (derivative instanceof EquityIndexOption) {
      ...
      return Collections.singleton(new ComputedValue(resultSpec, model.vomma((EquityIndexOption) derivative, market)));
    }
    final EquityOptionBlackMethod model = EquityOptionBlackMethod.getInstance();
    return Collections.singleton(new ComputedValue(resultSpec, model.vomma((EquityOption) derivative, market)));
  }
}
