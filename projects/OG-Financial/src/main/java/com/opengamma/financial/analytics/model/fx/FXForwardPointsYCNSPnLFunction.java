 ...
 *
 */
public class FXForwardPointsYCNSPnLFunction extends AbstractFunction.NonCompiledInvoker {
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target) {
    final ValueProperties properties = createValueProperties()
          ...
          .with(PROPERTY_PNL_CONTRIBUTIONS, YIELD_CURVE_NODE_SENSITIVITIES)
          .withAny(CURRENCY)
          .withAny(CURVE)
          .get();
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target, final Map<ValueSpecification, ValueRequirement> inputs) {
    ...
    for (final Map.Entry<ValueSpecification, ValueRequirement> entry : inputs.entrySet()) {
      ...
      if (valueName.equals(YIELD_CURVE_NODE_SENSITIVITIES)) {
        currency = key.getProperty(CURRENCY);
        curveExposures = key.getProperty(CURVE_EXPOSURES);
        forwardCurveName = key.getProperty(FORWARD_CURVE_NAME);
        curveName = key.getProperty(CURVE);
