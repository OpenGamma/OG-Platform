...
public class HistoricalTimeSeriesSecurityFunction extends AbstractFunction.NonCompiledInvoker {
  ...
  public Set<ComputedValue> execute(final FunctionExecutionContext executionContext, final FunctionInputs inputs,
    ...
    return Collections.singleton(new ComputedValue(
        new ValueSpecification(desiredValue.getValueName(), desiredValue.getTargetSpecification(), desiredValue.getConstraints()), mhts.getTimeSeries()));
  }
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target) {
    ...
    return Collections.singleton(new ValueSpecification(
        new ValueRequirement(ValueRequirementNames.HISTORICAL_TIME_SERIES, target.getSecurity(), props), getUniqueId()));
  }
