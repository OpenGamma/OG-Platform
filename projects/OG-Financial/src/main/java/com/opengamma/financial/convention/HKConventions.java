...
public class HKConventions {
  ...
  public static synchronized void addFixedIncomeInstrumentConventions(final ConventionBundleMaster conventionMaster) {
    ...
    for (int i = 1; i < 4; i++) {
      ...
      utils.addConventionBundle(ExternalIdBundle.of(bbgId, simpleId), name, ACT_365, FOLLOWING, DateUtils.periodOfDays(i), 0, false, HK);
    }
    for (int i = 1; i < 4; i++) {
      final String name = "HKD Deposit " + i + "w";
      final ExternalId bbgId = bloombergTickerSecurityId("HDDR" + i + BBG_WEEK_CODE + " Curncy");
      final ExternalId simpleId = simpleNameSecurityId(name);
      utils.addConventionBundle(ExternalIdBundle.of(bbgId, simpleId), name, ACT_365, FOLLOWING, DateUtils.periodOfDays(i * 7), 0, false, HK);
    }
    for (int i = 1; i < 12; i++) {
      final String name = "HKD Deposit " + i + "m";
      final ExternalId bbgId = bloombergTickerSecurityId("HDDR" + BBG_MONTH_CODES[i - 1] + " Curncy");
      final ExternalId simpleId = simpleNameSecurityId(name);
      utils.addConventionBundle(ExternalIdBundle.of(bbgId, simpleId), name, ACT_365, FOLLOWING, DateUtils.periodOfMonths(i), 0, false, HK);
    }
  }
}
