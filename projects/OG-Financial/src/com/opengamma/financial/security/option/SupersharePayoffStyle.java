...
// Created from com/opengamma/financial/security/option/PayoffStyle.proto:112(10)
...
public class SupersharePayoffStyle extends com.opengamma.financial.security.option.PayoffStyle implements java.io.Serializable {
  ...
  protected SupersharePayoffStyle (final org.fudgemsg.FudgeMsg fudgeMsg) {
    super (fudgeMsg);
  ...
  public static SupersharePayoffStyle fromFudgeMsg (final org.fudgemsg.FudgeMsg fudgeMsg) {
    final java.util.List<org.fudgemsg.FudgeField> types = fudgeMsg.getAllByOrdinal (0);
    for (org.fudgemsg.FudgeField field : types) {
      final String className = (String)field.getValue ();
      if ("com.opengamma.financial.security.option.SupersharePayoffStyle".equals (className)) break;
      try {
        return (com.opengamma.financial.security.option.SupersharePayoffStyle)Class.forName (className).getDeclaredMethod ("fromFudgeMsg", org.fudgemsg.FudgeMsg.class).invoke (null, fudgeMsg);
      }
      catch (Throwable t) {
        // no-action
      }
    }
    return new SupersharePayoffStyle (fudgeMsg);
  }
