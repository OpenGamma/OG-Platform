...
public class CurveDefinitionAndSpecifications {
  ...
  public static YieldCurveDefinition buildSingleCurve(final Currency ccy, final ExternalId region, final Tenor[] liborStrips, final Tenor futureStartTenor, final int numQuarterlyFutures,
      final Tenor[] swaps) {
    final Collection<FixedIncomeStrip> strips = new ArrayList<FixedIncomeStrip>();
    for (final Tenor liborTenor : liborStrips) {
      strips.add(new FixedIncomeStrip(StripInstrumentType.LIBOR, liborTenor, "DEFAULT"));
   ...
   * @param basisSwapsEndContiguousYears maximum number of years that contiguous basis swap rates are available, so 20 => 1..20
   * @param basisSwapsNonContiguousYears array of the non-contiguous basis swap rate years (e.g. new int[] { 15, 20, 30, 40 })
   * @param oisSwapMonths array of 1-based months for which OIS rates are available
   * @param oisSwapYears array of years for which OIS rates are available
