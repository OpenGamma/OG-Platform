 ...
 * 
 */
public class FXForwardCurveFromYieldCurveFunction extends AbstractFunction {
    ...
    return new AbstractInvokingCompiledFunction(atInstant.withTime(0, 0), atInstant.plusDays(1).withTime(0, 0).minusNanos(1000000)) {
      ...
      public Set<ValueRequirement> getRequirements(final FunctionCompilationContext context, final ComputationTarget target, final ValueRequirement desiredValue) {
        ...
        final ValueProperties receiveCurveProperties = ValueProperties.builder()
            ...
            .get();
        final UnorderedCurrencyPair ccyPair = (UnorderedCurrencyPair) target.getValue();
      ...
      public Set<ComputedValue> execute(final FunctionExecutionContext executionContext, final FunctionInputs inputs, final ComputationTarget target, final Set<ValueRequirement> desiredValues) {
        ...
        for (final ComputedValue input : inputs.getAllValues()) {
          ...
          if (properties.getValues(ValuePropertyNames.PAY_CURVE) != null) {
            payCurveObject = input.getValue();
            payCurveNames = properties.getValues(ValuePropertyNames.CURVE);
          } else if (properties.getValues(ValuePropertyNames.RECEIVE_CURVE) != null) {
        ...
        final ForwardCurve fxForwardCurve = new ForwardCurve(spot, payCurve, receiveCurve);
        final ValueProperties properties = createValueProperties()
