...
public class FXOptionVolatilitySurfaceConfigPopulator {
  ...
  private static void populateVolatilitySurfaceDefinitions(final ConfigMaster configMaster) {
    ...
    final VolatilitySurfaceDefinition<Tenor, Pair<Integer, FXVolQuoteType>> volSurfaceDefinition =
        new VolatilitySurfaceDefinition<Tenor, Pair<Integer, FXVolQuoteType>>("DEFAULT", Currency.USD, expiryTenors, deltaAndTypes);
    ConfigMasterUtils.storeByName(configMaster, makeConfigDocument(volSurfaceDefinition));
  }

  private static ConfigDocument<VolatilitySurfaceDefinition<Tenor, Pair<Integer, FXVolQuoteType>>> makeConfigDocument(final VolatilitySurfaceDefinition<Tenor, Pair<Integer, FXVolQuoteType>> definition) {
  ...
  private static void populateVolatilitySurfaceSpecifications(final ConfigMaster configMaster) {
    final SurfaceInstrumentProvider<Tenor, Pair<Integer, FXVolQuoteType>> surfaceInstrumentProvider = new BloombergFXOptionVolatilitySurfaceInstrumentProvider("USDJPY", "Curncy",
        MarketDataRequirementNames.MARKET_VALUE);
    final VolatilitySurfaceSpecification usdJPY = new VolatilitySurfaceSpecification("DEFAULT_USD_JPY", Currency.USD, surfaceInstrumentProvider);
    ConfigMasterUtils.storeByName(configMaster, makeConfigDocument(usdJPY));
  }
}
