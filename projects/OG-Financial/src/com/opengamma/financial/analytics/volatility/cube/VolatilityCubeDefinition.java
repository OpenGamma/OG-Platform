...
public class VolatilityCubeDefinition extends DirectBean implements UniqueIdentifiable, MutableUniqueIdentifiable {
  ...
  public Iterable<VolatilityPoint> getAllPoints() {
    int size = _swapTenors.size() * _optionExpiries.size() * _relativeStrikes.size();
    List<VolatilityPoint> ret = new ArrayList<VolatilityPoint>(size);
    for (Tenor swapTenor : _swapTenors) {
      for (Tenor optionExpiry : _optionExpiries) {
        {
          for (Double relativeStrike : _relativeStrikes) {
            ret.add(new VolatilityPoint(swapTenor, optionExpiry, relativeStrike.doubleValue()));
          }
        }
      }
    }
