...
public class NettingFixedCashFlowFunction extends AbstractFunction.NonCompiledInvoker {
  ...
  public Set<ComputedValue> execute(final FunctionExecutionContext executionContext, final FunctionInputs inputs, final ComputationTarget target,
    ...
    if (inputs.getAllValues().isEmpty()) {
      cashFlows = new TreeMap<LocalDate, MultipleCurrencyAmount>(definition.accept(NETTING_CASH_FLOW_CALCULATOR));
    } else {
      ...
      if (fixingSeries == null) {
        ...
        cashFlows = new TreeMap<LocalDate, MultipleCurrencyAmount>(definition.accept(NETTING_CASH_FLOW_CALCULATOR, fixingSeries.getTimeSeries()));
      }
    }
    return Collections.singleton(new ComputedValue(new ValueSpecification(ValueRequirementNames.NETTED_FIXED_CASH_FLOWS, target.toSpecification(), createValueProperties().get()),
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target) {
    return Collections.singleton(new ValueSpecification(ValueRequirementNames.NETTED_FIXED_CASH_FLOWS, target.toSpecification(), createValueProperties().get()));
  }
