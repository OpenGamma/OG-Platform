 ...
 * 
 */
public class FirstOfYearScheduleCalculatorTest {
  private static final Schedule CALCULATOR = new FirstOfYearScheduleCalculator();
  ...
  public void testStartAndEndSameButInvalid1() {
    CALCULATOR.getSchedule(LocalDate.of(2001, 1, 3), LocalDate.of(2001, 1, 3), false);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testStartAndEndSameButInvalid2() {
    CALCULATOR.getSchedule(LocalDate.of(2001, 2, 1), LocalDate.of(2001, 2, 1), false);
  }
  ...
  public void test() {
    ...
    assertEquals(backward[0], LocalDate.of(2003, 1, 1));
    ...
    for (int i = 1; i < months; i++) {
      if (forward[i].getYear() == forward[i - 1].getYear()) {
        ...
        assertEquals(backward[i].getMonthOfYear(), 1);
