...
public class CurrencyMatrixTest extends FinancialTestBase {
  ...
  public void testSymmetricalMatrix() {
    ...
    simple.setLiveData(Currency.getInstance("USD"), Currency.getInstance("GBP"), UniqueIdentifier.of(SecurityUtils.BLOOMBERG_TICKER.getName(), "GBP Curncy"));
    simple.setFixedConversion(Currency.getInstance("GBP"), Currency.getInstance("EUR"), 0.9);
    simple.setCrossConversion(Currency.getInstance("USD"), Currency.getInstance("EUR"), Currency.getInstance("GBP"));
    simple.setFixedConversion(Currency.getInstance("EUR"), Currency.getInstance("CHF"), 10.0);
  ...
  private static class RandomMatrix extends AbstractCurrencyMatrix {
    ...
    private static CurrencyMatrixValue randomValue(final Random r) {
      switch (r.nextInt(4)) {
        ...
        case 2:
          return CurrencyMatrixValue.of(Currency.getInstance("AA" + (char) ('A' + r.nextInt(('Z' - 'A') + 1))));
        case 3:
    ...
    public RandomMatrix() {
      ...
      for (char source = 'A'; source < 'Z'; source++) {
        final Currency sourceCurrency = Currency.getInstance("AA" + source);
        for (char target = (char) (source + 1); target <= 'Z'; target++) {
          final Currency targetCurrency = Currency.getInstance("AA" + target);
