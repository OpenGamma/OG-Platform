...
public class SABRSurfaceFittingTest {
  ...
  public void doIt() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      DoubleMatrix1D params = fitSABR(forwards[i], maturities[i], new DoubleMatrix1D(strikes), new DoubleMatrix1D(truckVols), new DoubleMatrix1D(errors), start, false);
  ...
  private DoubleMatrix1D fitSABR(final double forward, final double t, final DoubleMatrix1D strikes, final DoubleMatrix1D vols, final DoubleMatrix1D errors, final DoubleMatrix1D start,
    ...
    LeastSquareResults lsRes = solver.solve(strikes, vols, errors, func, transformParameters(start));
