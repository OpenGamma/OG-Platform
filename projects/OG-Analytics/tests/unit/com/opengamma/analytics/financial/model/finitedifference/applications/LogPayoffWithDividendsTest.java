...
public class LogPayoffWithDividendsTest {
  ...
  private static final double FLAT_PURE_VOL = 0.3;
  ...
  static {
    ...
    Function<Double, Double> localVol = new Function<Double, Double>() {
      ...
      public Double evaluate(Double... ts) {
        ...
        return FLAT_PURE_VOL * (s - d) / s;
      }
    };

    LOCAL_VOL = new LocalVolatilitySurfaceStrike(FunctionalDoublesSurface.from(localVol));

    PURE_LOCAL_VOL = new LocalVolatilitySurfaceMoneyness(ConstantDoublesSurface.from(FLAT_PURE_VOL), new ForwardCurve(1.0));
    ...
    final Function<Double, Double> pPrice = new Function<Double, Double>() {
      ...
      public Double evaluate(Double... tx) {
        ...
        final double pp = BlackFormulaRepository.price(1.0, x, t, FLAT_PURE_VOL, isCall);
        return pp;
      }
    };
  ...
  public void staticReplicationTest() {
    ...
    assertEquals(FLAT_PURE_VOL, Math.sqrt(-2 * val1 / EXPIRY), 1e-9);
  ...
  public void backwardsPDEtest() {
    ...
    ConvectionDiffusionPDEDataBundle pdeBundle = getBackwardsPDEDataBundle(EXPIRY, PURE_LOCAL_VOL, PURE_LOG_PAY_OFF);
    ...
    assertEquals(FLAT_PURE_VOL, Math.sqrt(-2 * val / EXPIRY), 1e-6);
  ...
  public void backwardsPDEtest2() {
    ...
    ConvectionDiffusionPDEDataBundle pdeBundle1 = getBackwardsPDEDataBundle(EXPIRY, LOCAL_VOL, payoff);
    ...
    ConvectionDiffusionPDEDataBundle pdeBundle2 = getBackwardsPDEDataBundle(EXPIRY, LOCAL_VOL, pricesAtDividend);
    ...
    assertEquals(FLAT_PURE_VOL, Math.sqrt(-2 * val2 / EXPIRY), 1e-6);
    //  System.out.println(val2 + "\t" + Math.sqrt(-2 * val2 / EXPIRY));
  }

  private ConvectionDiffusionPDEDataBundle getBackwardsPDEDataBundle(final double maturity, final LocalVolatilitySurfaceMoneyness localVol,
  ...
  private ConvectionDiffusionPDEDataBundle getBackwardsPDEDataBundle(final double maturity, final LocalVolatilitySurfaceStrike localVol,
      final Function1D<Double, Double> payoff) {
