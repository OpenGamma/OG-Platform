...
public class BlackSensitivityFromSABRSensitivityCalculatorTest {
  ...
  public ObjectsPair<SABRInterestRateParameters, HashMap<DoublesPair, DoubleMatrix2D>> calibration(final double[][][] volBlack) {
    double[] expiryTimeVector = new double[NB_EXPIRY * NB_MATURITY];
    double[] maturityTimeVector = new double[NB_EXPIRY * NB_MATURITY];
    double[] alphaVector = new double[NB_EXPIRY * NB_MATURITY];
    double[] betaVector = new double[NB_EXPIRY * NB_MATURITY];
    double[] rhoVector = new double[NB_EXPIRY * NB_MATURITY];
    double[] nuVector = new double[NB_EXPIRY * NB_MATURITY];
    int vect = 0;
    HashMap<DoublesPair, DoubleMatrix2D> inverseJacobianMap = new HashMap<DoublesPair, DoubleMatrix2D>();
    for (int loopexpiry = 0; loopexpiry < NB_EXPIRY; loopexpiry++) {
      ZonedDateTime settleDate = ScheduleCalculator.getAdjustedDate(EXPIRY_DATE[loopexpiry], USD6MLIBOR3M.getSpotLag(), NYC);
      for (int loopmat = 0; loopmat < NB_MATURITY; loopmat++) {
        SwapFixedIborDefinition swapDefinition = SwapFixedIborDefinition.from(settleDate, MATURITY_TENOR[loopmat], USD6MLIBOR3M, 1.0, 0.0, true); // used to compute atm
        SwapFixedCoupon<Coupon> swap = swapDefinition.toDerivative(REFERENCE_DATE, CURVE_NAMES);
        double atm = PRC.visit(swap, CURVES);
        double[] strikeAbs = new double[NB_STRIKE];
        double[] errors = new double[NB_STRIKE];
    ...
    SABRInterestRateParameters sabrParameters = new SABRInterestRateParameters(alphaSurface, betaSurface, rhoSurface, nuSurface, USD6MLIBOR3M.getFixedLegDayCount(), SABR_FUNCTION);
  ...
  public void blackNodeSensitivity() {
    ...
    double bump = 0.001;
    double[][][] sensiBlackFD = new double[NB_EXPIRY][NB_MATURITY][NB_STRIKE];
    ...
    for (int loopins = 0; loopins < NB_INS; loopins++) {
      ...
      for (int loopexpiry = 0; loopexpiry < NB_EXPIRY; loopexpiry++) {
        for (int loopmat = 0; loopmat < NB_MATURITY; loopmat++) {
          for (int loopstr = 0; loopstr < NB_STRIKE; loopstr++) {
            double[][][] bumpedVol = bump(loopexpiry, loopmat, loopstr, bump);
            ObjectsPair<SABRInterestRateParameters, HashMap<DoublesPair, DoubleMatrix2D>> bumpedResult = calibration(bumpedVol);
            SABRInterestRateDataBundle bumpedSabrBundle = new SABRInterestRateDataBundle(bumpedResult.getFirst(), CURVES);
            double bumpedPv = METHOD_SWAPTION_SABR.presentValue(INSTRUMENTS[loopins], bumpedSabrBundle).getAmount();
            sensiBlackFD[loopexpiry][loopmat][loopstr] = (bumpedPv - pv[loopins]) / bump;
            double sensiCalc = sensiBlack.get(DoublesPair.of(EXPIRY_TIME[loopexpiry], MATURITY_TIME[loopmat])).getEntry(loopstr);
  ...
  private double[][][] bump(int exp, int mat, int str, double bump) {
    double[][][] vol = new double[NB_EXPIRY][NB_MATURITY][NB_STRIKE];
    for (int loopexpiry = 0; loopexpiry < NB_EXPIRY; loopexpiry++) {
      for (int loopmat = 0; loopmat < NB_MATURITY; loopmat++) {
        for (int loopstr = 0; loopstr < NB_STRIKE; loopstr++) {
