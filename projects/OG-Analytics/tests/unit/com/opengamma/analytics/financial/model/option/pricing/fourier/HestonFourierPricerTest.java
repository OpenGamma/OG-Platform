...
import com.opengamma.analytics.financial.model.option.pricing.fourier.FourierModelGreeks;
...
public class HestonFourierPricerTest {
  ...
  public void testHestonCE() {
    ...
    for (int i = 0; i < 101; i++) {
      ...
      for (int j = 0; j < 101; j++) {
        double y = -3. + j * 6. / 100.0;
        ComplexNumber res = heston.getValue(new ComplexNumber(x, y), 0.25);
  ...
  public void testEuropeanCallFTHeston() {
    ...
    Function1D<ComplexNumber, ComplexNumber> func = integrand.getFunction(t);

    for (int i = 0; i < 201; i++) {
      double x = -0. + i * 20. / 200.0;
      ComplexNumber res = func.evaluate((new ComplexNumber(x, alpha)));
  ...
  public void testHestonModelGreeks() {
    ...
    FourierModelGreeks modelGreek = new FourierModelGreeks();
    ...
    for (int i = 0; i < 11; i++) {
      ...
      double[] sense = modelGreek.getGreeks(data, option, heston, alpha, 1e-12);

      final double price = pricer.price(data, option, heston, alpha, 1e-8);
      double[] fdSense = finiteDifferenceModelGreeks((HestonCharacteristicExponent) heston, pricer, data, option);
...
//          + "\t" + sense[0] / price + "\t" + sense[1] / price + "\t" + sense[2] / price + "\t" + sense[3] / price + "\t" + sense[4] / price);
  ...
  public void testHestonModelGreeksIntegrand() {
    ...
    for (int i = 0; i < 201; i++) {
      ...
      for (int index = 0; index < 5; index++) {
        double value = funcs.get(index).evaluate(x);
