...
public class ISDATestGridManager {
  ...
  private static final Logger s_logger = LoggerFactory.getLogger(ISDATestGridManager.class);
  ...
  public String[] findAllTestGridFiles() throws URISyntaxException {

    URL directory = getClass().getClassLoader().getResource(s_testGridFileLocation);

    if (directory == null) {
      throw new RuntimeException("ISDA test grid directory not found: " + s_testGridFileLocation);
    }
  ...
  public ISDATestGrid loadTestGrid(final String fileName) throws IOException {

    InputStream is = null;

    try {
      is = getClass().getClassLoader().getResourceAsStream(s_testGridFileLocation + fileName);
  ...
  //			JAXBException {
  //
  //		for (int i = 0; i < ISDATestGridManager.testGrids.length; i++) {
  //			String fileName = s_testGridFileLocation
  //					+ ISDATestGridManager.testGrids[i];
  //			s_logger.info("Finding file " + fileName);
  //			InputStream is = ISDATestGridManager.getFileInputStream(fileName);
  //
  //			try {
  ...
  //				Workbook wb = new HSSFWorkbook(is);
  //				ISDATestGrid testGrid = ISDATestGridManager.getTestGrid(wb);
  ...
  //		return ISDATestGridManager.class.getClassLoader().getResourceAsStream(
  //				fileName);
  //	}
  //	
  //	public static ISDATestGrid getTestGrid(String fileName) throws IOException, InterruptedException, JAXBException {
  //	  InputStream is = ISDATestGridManager.getFileInputStream(s_testGridFileLocation + fileName);
  //	  Workbook wb = new HSSFWorkbook(is);
  //	  return ISDATestGridManager.getTestGrid(wb);
  //	}
  //
  //	public static ISDATestGrid getTestGrid(Workbook wb)
