...
public class SABRBlackEquivalentVolatilitySurfaceModelTest {
  ...
  public void testBetaApproachesZero() {
    SABRDataBundle data = DATA.withBeta(0 - 1e-12);
    assertEquals(MODEL.getSurface(OPTION, DATA.withBeta(0)).getVolatility(PAIR), MODEL.getSurface(OPTION, data).getVolatility(PAIR), 1e-9);
    data = DATA.withBeta(0 + 1e-12);
    assertEquals(MODEL.getSurface(OPTION, DATA.withBeta(0)).getVolatility(PAIR), MODEL.getSurface(OPTION, data).getVolatility(PAIR), 1e-9);
  }
