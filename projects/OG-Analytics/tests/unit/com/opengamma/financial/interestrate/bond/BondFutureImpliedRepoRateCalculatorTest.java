...
public class BondFutureImpliedRepoRateCalculatorTest {
  ...
  private static final BondForward[] DELIVERABLES = new BondForward[] {new BondForward(new Bond(new double[] {1, 2, 3}, COUPONS[0], NAME), DELIVERY_TIMES[0], ACCRUED_INTEREST[0]),
      new BondForward(new Bond(new double[] {1, 2, 3, 4, 5, 6}, COUPONS[1], NAME), DELIVERY_TIMES[1], ACCRUED_INTEREST[1]),
      new BondForward(new Bond(new double[] {1, 2, 3, 4, 5}, COUPONS[2], NAME), DELIVERY_TIMES[2], ACCRUED_INTEREST[2])};
  ...
  public void test() {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      invoicePrice[i] = FUTURE_PRICE * CONVERSION_FACTORS[i] + ACCRUED_INTEREST[i];
