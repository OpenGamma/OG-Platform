...
public class PaymentFixedDefinitionTest {
  ...
  private static final PaymentFixedDefinition FIXED_PAYMENT = new PaymentFixedDefinition(PAYMENT_DATE, AMOUNT);
  ...
  public void testEqualHashCode() {
    PaymentFixedDefinition comparedPayment = new PaymentFixedDefinition(PAYMENT_DATE, AMOUNT);
    assertEquals(FIXED_PAYMENT, comparedPayment);
    assertEquals(FIXED_PAYMENT.hashCode(), comparedPayment.hashCode());
    PaymentFixedDefinition modifiedPayment = new PaymentFixedDefinition(PAYMENT_DATE, AMOUNT + 1.0);
    assertFalse(FIXED_PAYMENT.equals(modifiedPayment));
    ZonedDateTime modifiedDate = PAYMENT_DATE.minusDays(1);
    modifiedPayment = new PaymentFixedDefinition(modifiedDate, AMOUNT);
    assertFalse(FIXED_PAYMENT.equals(modifiedPayment));
  }
