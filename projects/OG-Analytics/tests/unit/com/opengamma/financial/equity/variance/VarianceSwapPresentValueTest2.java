 ...
 * 
 */
public class VarianceSwapPresentValueTest2 {
  ...
  // The pricing method
  final VarianceSwapStaticReplication2<Strike> pricer_default_w_cutoff = new VarianceSwapStaticReplicationStrike(new Strike(1e-10*SPOT),new Strike(100*SPOT),new RungeKuttaIntegrator1D(),new Strike(0.25*SPOT),
      new Strike(0.05*SPOT));
  final VarianceSwapStaticReplication2<Strike> pricer_without_cutoff = new VarianceSwapStaticReplicationStrike();
  ...
  private static final VarianceSwapDataBundle2<Strike> MARKET = new VarianceSwapDataBundle2<Strike>(VOL_SURFACE, DISCOUNT, FORWARD_CURVE);
  ...
  public void onFirstObsDateWithOneObs() {
    ...
    final double pv = pricer_without_cutoff.presentValue(swapStartsNow, MARKET);
    final double variance = pricer_without_cutoff.impliedVariance(swapStartsNow, MARKET);
  ...
  public void swapStartsTomorrow() {
    ...
    final double pvStartsObsTomorrow = pricer_without_cutoff.presentValue(swapStartsTomorrow, MARKET);
    final double pvStartsObsToday = pricer_without_cutoff.presentValue(swapStartsNow, MARKET);
  ...
  public void swapForwardStarting() {
    ...
    final double pvFowardStart = pricer_without_cutoff.presentValue(swapForwardStarting1to5, MARKET);
    ...
    final double pvSpot1 = pricer_without_cutoff.presentValue(swapSpotStarting1, MARKET);
    final double pvSpot5 = pricer_without_cutoff.presentValue(swapSpotStarting5, MARKET);
  ...
  public void onFirstObsWithoutObs() {
    ...
    final VarianceSwap swapOnFirstObsWithoutObs = new VarianceSwap(now, expiry5, expiry5, varStrike, varNotional, Currency.EUR, annualization, nObsExpected, noObsDisrupted, noObservations,
        noObsWeights);
    @SuppressWarnings("unused")
    final double pv = pricer_without_cutoff.presentValue(swapOnFirstObsWithoutObs, MARKET);
  }
  ...
  public void swapObservationsCompleted() {
    ...
    final double pv = pricer_without_cutoff.presentValue(swapPaysTomorrow, MARKET);
  ...
  public void swapAfterSettlement() {
    ...
    final double pv = pricer_without_cutoff.presentValue(swapEnded, MARKET);
