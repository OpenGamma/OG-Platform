...
### @export "read-data"
data = read.table("dexy--sabr-extrapolation-data.txt", header=TRUE)
data$Mu <- as.factor(data$Mu)

### @export "calculate-density"
# This needs to be made more R-like...
...
n <- length(x1)
Density1 <- c(NA, (x1[3:n] + x1[1:(n-2)] - 2 * x1[2:(n-1)])/(y1[2:(n-1)] - y1[1:(n-2)])^2, NA)
Density2 <- c(NA, (x2[3:n] + x2[1:(n-2)] - 2 * x2[2:(n-1)])/(y2[2:(n-1)] - y2[1:(n-2)])^2, NA)
Density3 <- c(NA, (x3[3:n] + x3[1:(n-2)] - 2 * x3[2:(n-1)])/(y3[2:(n-1)] - y3[1:(n-2)])^2, NA)
Density4 <- c(NA, (x4[3:n] + x4[1:(n-2)] - 2 * x4[2:(n-1)])/(y4[2:(n-1)] - y4[1:(n-2)])^2, NA)
...
pdf("dexy--extrapolation-price.pdf", width=16)
with(data,
xyplot(Price ~ Strike, groups=Mu, type="l", lwd=2,
        xlab=list(cex=2), ylab=list(cex=2), scales=list(x=list(cex=2), y=list(cex=2)),
        ylim=c(0, 0.0004), xlim=c(0.08,0.16),
...
with(data,
xyplot(ImpliedVol ~ Strike, groups=Mu, type="l", lwd=2,
        xlab=list(cex=2), ylab=list(cex=2), scales=list(x=list(cex=2), y=list(cex=2)),
        auto.key = list(cex=2, text = paste("mu =", levels(Mu)),
...
pdf("dexy--extrapolation-smile.pdf", width=16)
with(data,
xyplot(ImpliedVol ~ Strike, groups=Mu, type="l", lwd=2, 
        xlab=list(cex=2), ylab=list(cex=2), scales=list(x=list(cex=2), y=list(cex=2)),
        auto.key = list(cex=2, text = paste("mu =", levels(Mu)),
...
pdf("dexy--extrapolation-density.pdf", width=16)
with(data,
xyplot(Density ~ Strike, groups=Mu, type="l", lwd=2, ylim=c(0, 0.5),
        xlab=list(cex=2), ylab=list(cex=2), scales=list(x=list(cex=2), y=list(cex=2)),
        auto.key = list(cex=2, text = paste("mu =", levels(Mu)),
