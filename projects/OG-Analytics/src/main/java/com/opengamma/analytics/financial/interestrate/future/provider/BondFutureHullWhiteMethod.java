...
import com.opengamma.analytics.financial.provider.description.HullWhiteIssuerProviderInterface;
...
public final class BondFutureHullWhiteMethod extends BondFutureMethod {
  ...
  public double price(final BondFuture future, final HullWhiteIssuerProviderInterface data, final int nbPoint) {
    ...
    Pair<String, Currency> issuerCcy = future.getDeliveryBasket()[0].getIssuerCcy();
    ...
    String issuerName = future.getDeliveryBasket()[0].getIssuer();
    HullWhiteOneFactorPiecewiseConstantParameters parameters = data.getHullWhiteParameters();
    IssuerProviderInterface issuer = data.getIssuerProvider();
    MulticurveProviderInterface multicurvesDecorated = new MulticurveProviderDiscountingDecoratedIssuer(issuer, future.getCurrency(), issuerName);
    ...
    for (int loopbnd = 0; loopbnd < nbBond; loopbnd++) {
      cf[loopbnd] = CFEC.visit(future.getDeliveryBasket()[loopbnd], multicurvesDecorated);
   ...
   * @return The curve sensitivity.
   */
  public MulticurveSensitivity priceCurveSensitivity(final BondFuture future, final HullWhiteIssuerProviderInterface data) {
