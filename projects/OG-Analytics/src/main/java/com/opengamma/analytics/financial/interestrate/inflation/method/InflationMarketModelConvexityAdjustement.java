 ...
 */
public class InflationMarketModelConvexityAdjustement {

  /**
   * Computes the convexity adjustment for year on year inflation swap.
   * @param firstFixingTime The
   * @param secondFixingTime The
   * @param firstNaturalPaymentTime The
   * @param secondNaturalPaymentTime The
   * @param paymentTime The
   * @param inflationConvexity The
   * @return The convexity adjustment.
   */
  public double getYearOnYearConvexityAdjustment(final double firstFixingTime, final double secondFixingTime, final double firstNaturalPaymentTime, final double secondNaturalPaymentTime,
      final double paymentTime, final InflationConvexityAdjustmentProviderInterface inflationConvexity) {
    ArgumentChecker.isTrue(firstFixingTime < secondFixingTime, null);
    ArgumentChecker.isTrue(firstFixingTime < firstNaturalPaymentTime, null);
    ArgumentChecker.isTrue(firstFixingTime < firstNaturalPaymentTime, null);
  ...
  /**
   * Computes the convexity adjustment for zero coupon inflation swap.
   * @param fixingTime The 
   * @param naturalPaymentTime The
   * @param paymentTime The
   * @param inflationConvexity The
   * @return The convexity adjustment.
   */
  public double getZeroCouponConvexityAdjustment(final double fixingTime, final double naturalPaymentTime, final double paymentTime,
      final InflationConvexityAdjustmentProviderInterface inflationConvexity) {
    ArgumentChecker.isTrue(fixingTime < naturalPaymentTime, null);
    ArgumentChecker.isTrue(naturalPaymentTime <= paymentTime, null);
