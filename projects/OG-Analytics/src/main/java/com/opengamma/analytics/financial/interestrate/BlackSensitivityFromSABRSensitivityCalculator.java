...
public class BlackSensitivityFromSABRSensitivityCalculator {
   ...
   */
  public static Map<DoublesPair, DoubleMatrix1D> blackSensitivity(PresentValueSABRSensitivityDataBundle sabrSensitivity, final Map<DoublesPair, DoubleMatrix2D> inverseJacobianMap) {
    Set<DoublesPair> alphaSet = sabrSensitivity.getAlpha().getMap().keySet();
    Set<DoublesPair> ijSet = inverseJacobianMap.keySet();
