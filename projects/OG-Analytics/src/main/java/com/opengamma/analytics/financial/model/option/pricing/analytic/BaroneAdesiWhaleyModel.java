...
public class BaroneAdesiWhaleyModel {
  ...
  public class CallSolver {
    ...
    private double getH(final double s) {
      final double d1 = getd1(s);
      double res = _df2 * NORMAL.getCDF(d1) * (1 - 1 / _q2) + (1 - _df2 * NORMAL.getPDF(d1) / _sigmaRootT) / _q2;
      return res;
    }
    ...
    private double getSStar(final double initalValue) {
      ...
      double d1 = getd1(s);
      double rhs = getBSPrice(s) + (1 - _df2 * NORMAL.getCDF(d1)) * s / _q2;
      double h;
      ...
      while (error > TOL && count < MAX_INT) {
        ...
        s = (_k + rhs - h * s) / (1 - h);
        d1 = getd1(s);
        rhs = getBSPrice(s) + (1 - _df2 * NORMAL.getCDF(d1)) * s / _q2;
    ...
    /**
     * The first order Greeks for BSM 
     * @param s spot level 
  ...
  private class PutSolver {
    ...
    public PutSolver(final double s0, final double k, final double r, final double b, final double t, final double sigma) {
      ...
      _inital = sInf + (k - sInf) * Math.exp(h1);
    }

    public double price() {
      final double sStar = getSStar(_inital);
      if (_s0 <= sStar) {
        return _k - _s0;
      } else {
        final double a1 = getA1(sStar);
        return getBSPrice(_s0) + a1 * Math.pow(_s0 / sStar, _q1);
      }
    }

    public double getd1(final double s) {
      ArgumentChecker.isTrue(s > 0, "s is negative");
      return (Math.log(s / _k) + _phi) / _sigmaRootT;
    }

    public double getA1(final double s) {
      final double d1 = getd1(s);
      double res = -s * (1 - _df2 * NORMAL.getCDF(-d1)) / _q1;
      return res;
    }

    public double geth(final double s) {
      final double d1 = getd1(s);
      double res = -_df2 * NORMAL.getCDF(-d1) * (1 - 1 / _q1) - (1 + _df2 * NORMAL.getPDF(-d1) / _sigmaRootT) / _q1;
      return res;
    }
    ...
    public double getSStar(final double initalValue) {

      double s = initalValue;
      double d1 = getd1(s);
      double rhs = getBSPrice(s) - (1 - _df2 * NORMAL.getCDF(-d1)) * s / _q1;
      double h = geth(s);
      ...
      while (error > TOL && count < MAX_INT) {
        s = (_k - rhs + h * s) / (1 + h);
        d1 = getd1(s);
        rhs = getBSPrice(s) - (1 - _df2 * NORMAL.getCDF(-d1)) * s / _q1;
        h = geth(s);
