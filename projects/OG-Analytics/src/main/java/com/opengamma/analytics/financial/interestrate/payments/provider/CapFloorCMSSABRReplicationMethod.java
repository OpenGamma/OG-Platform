...
public class CapFloorCMSSABRReplicationMethod extends CapFloorCMSSABRReplicationAbstractMethod {
  ...
  public MultipleCurrencyAmount presentValue(final CapFloorCMS cmsCapFloor, final SABRSwaptionProviderInterface sabrData) {
    ...
    final double forward = PRDC.visit(underlyingSwap, sabrData.getMulticurveProvider());
  ...
  public MultipleCurrencyMulticurveSensitivity presentValueCurveSensitivity(final CapFloorCMS cmsCapFloor, final SABRSwaptionProviderInterface sabrData) {
    ...
    MulticurveSensitivity dscDp = MulticurveSensitivity.ofYieldDiscounting(resultMapDsc);
    MulticurveSensitivity forwardDp = PRCSDC.visit(cmsCapFloor.getUnderlyingSwap(), sabrData.getMulticurveProvider());
  ...
  public double presentValueStrikeSensitivity(final CapFloorCMS cmsCapFloor, final SABRSwaptionProviderInterface sabrData) {
    ...
    Function1D<SABRFormulaData, double[]> sabrFunctionAdjoint = sabrParameter.getSabrFunction().getVolatilityAdjointFunction(option, forward);
    double[] volA = sabrFunctionAdjoint.evaluate(sabrPoint);
    final BlackFunctionData dataBlack = new BlackFunctionData(forward, 1.0, volA[0]);
    final BlackPriceFunction blackFunction = new BlackPriceFunction();
    double[] bsA = blackFunction.getPriceAdjoint(option, dataBlack);
    ...
    double secondPart = integrant.k(strike) * (bsA[3] + bsA[2] * volA[2]);
  ...
  private class CMSStrikeIntegrant extends CMSIntegrant {
     ...
     * @param forward
     */
    public CMSStrikeIntegrant(CapFloorCMS cmsCap, SABRInterestRateParameters sabrParameter, double forward) {
      super(cmsCap, sabrParameter, forward);
    }
