...
import com.opengamma.analytics.financial.provider.sensitivity.ParameterSensitivityMatrixProviderCalculator;
 ...
 */
public class ProviderDiscountBuildingRepository {
   ...
   */
  public ProviderDiscountBuildingRepository(double toleranceAbs, double toleranceRel, int stepMaximum) {
  ...
  private Pair<MulticurveProviderDiscount, Double[]> makeUnit(InstrumentDerivative[] instruments, double[] initGuess, final MulticurveProviderDiscount knownData,
      ...
      final InstrumentDerivativeVisitor<MulticurveProviderInterface, CurveSensitivityMarket> sensitivityCalculator) {
    final GeneratorProviderDiscount generator = new GeneratorProviderDiscount(knownData, discountingMap, forwardIborMap, forwardONMap, generatorsMap);
    final ProviderDiscountBuildingData data = new ProviderDiscountBuildingData(instruments, generator);
    final Function1D<DoubleMatrix1D, DoubleMatrix1D> curveCalculator = new ProviderDiscountFinderFunction(calculator, data);
    final Function1D<DoubleMatrix1D, DoubleMatrix2D> jacobianCalculator = new ProviderDiscountFinderJacobian(new ParameterSensitivityMatrixProviderCalculator(sensitivityCalculator), data);
  ...
  private DoubleMatrix2D[] makeCurveMatrix(InstrumentDerivative[] instruments, int startBlock, int[] nbParameters, Double[] parameters, final MulticurveProviderDiscount knownData,
      final LinkedHashMap<String, Currency> discountingMap, final LinkedHashMap<String, IborIndex> forwardIborMap, final LinkedHashMap<String, IndexON> forwardONMap,
      final LinkedHashMap<String, GeneratorYDCurve> generatorsMap, final InstrumentDerivativeVisitor<MulticurveProviderInterface, CurveSensitivityMarket> sensitivityCalculator) {
    final GeneratorProviderDiscount generator = new GeneratorProviderDiscount(knownData, discountingMap, forwardIborMap, forwardONMap, generatorsMap);
    final ProviderDiscountBuildingData data = new ProviderDiscountBuildingData(instruments, generator);
    final Function1D<DoubleMatrix1D, DoubleMatrix2D> jacobianCalculator = new ProviderDiscountFinderJacobian(new ParameterSensitivityMatrixProviderCalculator(sensitivityCalculator), data);
