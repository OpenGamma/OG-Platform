...
public class CapletStrippingAbsoluteStrikePSpline extends CapletStrippingAbsoluteStrike {
  ...
  public CapletStrippingAbsoluteStrikePSpline(List<CapFloor> caps, YieldCurveBundle yieldCurves) {
    ...
    double[] t = getPricer().getCapletExpiries();
...
//  }
  ...
  public CapletStrippingSingleStrikeResult solveForPrices(double[] capPrices) {
    ...
    double[] error = new double[getnCaps()];
  ...
  public CapletStrippingSingleStrikeResult solveForPrices(double[] capPrices, double[] errors, boolean scaleByVega) {
    ...
    VolatilityTermStructure volCurve = getVolCurve(lsRes.getFitParameters());
    double[] mPrices = pricer.price(volCurve);
    ...
    double chi2 = chiSqr(capPrices, mPrices, errors); // ignore the vega weighting here
  ...
  public CapletStrippingSingleStrikeResult solveForVol(double[] capVols, double[] errors, boolean solveViaPrice) {
    ...
    if (solveViaPrice) {
      ...
      final Function1D<DoubleMatrix1D, DoubleMatrix1D> modelVolFunc = new Function1D<DoubleMatrix1D, DoubleMatrix1D>() {
        ...
        public DoubleMatrix1D evaluate(DoubleMatrix1D x) {
          double[] modelVols = pricer.impliedVols(_volModel.evaluate(x));
    ...
    VolatilityTermStructure volCurve = getVolCurve(lsRes.getFitParameters());
    double[] mVols = pricer.impliedVols(volCurve);

    // least-squares gives chi2 including the penalty, and is calculated via the price differecne and vega w we just want the fit error
    // TODO maybe the solver should provide this?
    double chi2 = chiSqr(capVols, mVols, errors);
  ...
  private LeastSquareResults solveForPrice(final double[] capPrices, final double[] capVols, final double[] errors, final boolean scaleByVega) {
    ...
    final Function1D<DoubleMatrix1D, DoubleMatrix1D> modelPriceFunc = new Function1D<DoubleMatrix1D, DoubleMatrix1D>() {
      @Override
      public DoubleMatrix1D evaluate(DoubleMatrix1D x) {
        double[] modelPrices = pricer.price(_volModel.evaluate(x));
