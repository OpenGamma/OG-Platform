 ...
 */
package com.opengamma.analytics.financial.provider.sensitivity.blackstirfutures;
...
public class ParameterSensitivityBlackSTIRFuturesExpLogMoneynessDiscountInterpolatedFDCalculator {
   ...
   */
  private final InstrumentDerivativeVisitor<BlackSTIRFuturesProviderInterface, MultipleCurrencyAmount> _valueCalculator;
  /**
  ...
  public MultipleCurrencyParameterSensitivity calculateSensitivity(final InstrumentDerivative instrument, final BlackSTIRFuturesExpLogMoneynessProviderDiscount black) {
    ...
    for (final Currency ccy : ccyDiscounting) {
      ...
      for (int loopnode = 0; loopnode < nbNodePoint; loopnode++) {
        ...
        final BlackSTIRFuturesExpLogMoneynessProviderDiscount marketDscBumpedMinus = new BlackSTIRFuturesExpLogMoneynessProviderDiscount(
            black.getMulticurveProvider().withDiscountFactor(ccy, dscBumpedMinus), black.getBlackParameters(), black.getFuturesIndex());
    ...
    for (final IndexON index : indexON) {
      ...
      for (int loopnode = 0; loopnode < nbNodePoint; loopnode++) {
        ...
        final YieldAndDiscountCurve fwdBumpedMinus = new YieldCurve(curveInt.getName(),
            new InterpolatedDoublesCurve(curveInt.getXDataAsPrimitive(), yieldBumpedMinus, curveInt.getInterpolator(), true));
        final BlackSTIRFuturesExpLogMoneynessProviderDiscount marketFwdBumpedMinus = new BlackSTIRFuturesExpLogMoneynessProviderDiscount(
            black.getMulticurveProvider().withForward(index, fwdBumpedMinus), black.getBlackParameters(), black.getFuturesIndex());
