 ...
 */
public class ProviderForwardBuildingRepository {
   ...
   */
  public ProviderForwardBuildingRepository(double toleranceAbs, double toleranceRel, int stepMaximum) {
  ...
  private Pair<MulticurveProviderForward, Double[]> makeUnit(InstrumentDerivative[] instruments, double[] initGuess, final MulticurveProviderForward knownData,
    ...
    final GeneratorProviderForward generator = new GeneratorProviderForward(knownData, discountingMap, forwardIborMap, forwardONMap, generatorsMap);
    final ProviderForwardBuildingData data = new ProviderForwardBuildingData(instruments, generator);
    final Function1D<DoubleMatrix1D, DoubleMatrix1D> curveCalculator = new ProviderForwardFinderFunction(calculator, data);
    final Function1D<DoubleMatrix1D, DoubleMatrix2D> jacobianCalculator = new ProviderForwardFinderJacobian(new ParameterSensitivityMatrixProviderCalculator(sensitivityCalculator), data);
