...
public class IndexOptionsPaperExamples extends ISDABaseTest {
  ...
  public void payoffVsIndexSpread() {
    ...
    final double indexSize = 100.;
    final double k = 0.012; //strike of 120bps
    final LocalDate optionExpiry = TRADE_DATE.plusMonths(1);
    final double offset = ACT365F.getDayCountFraction(TRADE_DATE, optionExpiry);
    final ISDACompliantYieldCurve fwdYC = YIELD_CURVE.withOffset(offset);
    ...
    final double fullAcc = fwdCDS.getAccruedPremium(coupon);

    final AnnuityForSpreadFunction annFunc = new AnnuityForSpreadISDAFunction(fwdCDS, fwdYC);
    final double excisePrice = (k - coupon) * annFunc.evaluate(k) - fullAcc;
    ...
    for (int i = 0; i < 200; i++) {
      ...
      final double indexValue = ((s - coupon) * annuity - fullAcc);
      System.out.print(s * TEN_THOUSAND);
      for (int j = 0; j < 5; j++) {
        final double f = (indexSize - defaults[j]) / indexSize;
        final double defaultedValue = (1 - RECOVERY_RATE) * defaults[j] / indexSize;
        //discount the payoff from the exercise settlement date to the exercise date
        final double payoff = df * Math.max(0, defaultedValue + f * indexValue - excisePrice);
    ...
    for (int i = 0; i < 200; i++) {
      ...
      for (int j = 0; j < 5; j++) {
        final double f = (indexSize - defaults[j]) / indexSize;
        final double defaultedValue = (1 - RECOVERY_RATE) * defaults[j] / indexSize;
        final double payoff = df * Math.max(0, excisePrice - defaultedValue - f * indexValue);
        System.out.print("\t" + payoff);
      }
      System.out.print("\n");
    }
  ...
  public void isdaVApproxTest() {
    ...
    final LocalDate esDate = addWorkDays(optionExpiry, 3, DEFAULT_CALENDAR);
    ...
    final CDSAnalytic fwdStartingCDS = FACTORY.makeCDS(TRADE_DATE, optionExpiry.plusDays(1), esDate, accStart, mat);
    final CDSAnalytic fwdCDS = FACTORY.makeIMMCDS(optionExpiry, Period.ofYears(5));
    ...
    final double indexFwd = INDEX_CAL.defaultAdjustedForwardIndexValue(fwdStartingCDS, timeToExpiry, INDEX_SIZE, YIELD_CURVE, INDEX_COUPON, adjCurves, RECOVERY_RATES);
    final MarketQuoteConverter converter = new MarketQuoteConverter();
    final double adjIndexFwd = indexFwd + fwdCDS.getAccruedPremium(INDEX_COUPON);
    final double atmFwd = converter.pufToQuotedSpread(fwdCDS, INDEX_COUPON, YIELD_CURVE.withOffset(timeToExpiry), adjIndexFwd);
    final double vol = 0.4;
    System.out.println("atmFwd: " + atmFwd * TEN_THOUSAND);
    System.out.println("Strike (bps)\tPrice (ISDA)\tPrice(approx)");
    for (int i = 0; i < 200; i++) {
      final double k = 0.009 + 0.014 * i / 199.;
      final boolean isPayer = k >= atmFwd;
      final double price1 = pricerISDA.getOptionPriceForSpreadQuotedIndex(indexFwd, vol, k, isPayer);
      final double price2 = pricerApprox.getOptionPriceForSpreadQuotedIndex(indexFwd, vol, k, isPayer);
  ...
  public void defaultedNamesTest() {
    ...
    final ISDACompliantCreditCurve[] adjCurves = PSA.adjustCurves(CDS_5Y, INDEX_COUPON, YIELD_CURVE, CREDIT_CURVES, RECOVERY_RATES, indexPrice);
    ...
    for (int i = 0; i < 100; i++) {
      ...
      for (int nDefaulted = 0; nDefaulted < maxDefaults; nDefaulted++) {
        ...
        for (int j = INDEX_SIZE - 1; j > INDEX_SIZE - 1 - nDefaulted; j--) {
          defaultedValue += (1 - RECOVERY_RATES[j]);
        }
        ...
        final double fwdVal = INDEX_CAL.defaultAdjustedForwardIndexValue(fwdStartingCDS, timeToExpiry, INDEX_SIZE, YIELD_CURVE, INDEX_COUPON, nonDefaultedAdjCurves, nonDefaultedRR, defaultedValue);
  ...
  public void blackTest() {
    ...
    final ISDACompliantCreditCurve indexCreditCurve = INDEX_CAL.impliedIndexCurve(pillarCDS, INDEX_COUPON, YIELD_CURVE, adjCurves, RECOVERY_RATES);
    final double price = PRICER.pv(CDS_5Y, YIELD_CURVE, indexCreditCurve, INDEX_COUPON);
    //System.out.println(price);
    assertEquals(indexPrice, price, 1e-16);

    final LocalDate optionExpiry = LocalDate.of(2014, 3, 19);
    final CDSAnalytic fwdCDS = FACTORY.makeIMMCDS(optionExpiry, Period.ofYears(5));
    final CDSAnalytic fwdStartingCDS = FACTORY.makeForwardStartingIMMCDS(TRADE_DATE, optionExpiry, Period.ofYears(5));
    final double timeToExpiry = ACT365F.getDayCountFraction(TRADE_DATE, optionExpiry);

    final IndexOptionPricer iop = new IndexOptionPricer(fwdCDS, timeToExpiry, YIELD_CURVE, INDEX_COUPON);
    final double fwdAnnuity = INDEX_CAL.indexAnnuity(fwdStartingCDS, YIELD_CURVE, adjCurves, RECOVERY_RATES, PriceType.CLEAN, 0.0);
    final double fwdSpread = INDEX_CAL.forwardSpread(fwdStartingCDS, timeToExpiry, INDEX_SIZE, YIELD_CURVE, adjCurves, RECOVERY_RATES, 0);
    final double dav = INDEX_CAL.defaultAdjustedForwardIndexValue(fwdStartingCDS, timeToExpiry, INDEX_SIZE, YIELD_CURVE, INDEX_COUPON, adjCurves, RECOVERY_RATES);
    final AnnuityForSpreadFunction annuityFunction = new AnnuityForSpreadISDAFunction(fwdCDS, YIELD_CURVE.withOffset(timeToExpiry));

    final double vol = 0.45;
    for (int i = 0; i < 100; i++) {
      final double k = 0.005 + 0.02 * i / 100.;
      final double p = ds.price(fwdStartingCDS, YIELD_CURVE, indexCreditCurve, k, timeToExpiry, vol, true, false);
      final double p2 = ds.priceMod(fwdStartingCDS, YIELD_CURVE, indexCreditCurve, k, timeToExpiry, vol, true, true, INDEX_COUPON);

      final double p3 = iop.getOptionPriceForSpreadQuotedIndex(dav, vol, k, true);
      final double kMod = INDEX_COUPON + (k - INDEX_COUPON) * annuityFunction.evaluate(k) / fwdAnnuity;
      final double p4 = fwdAnnuity * BlackFormulaRepository.price(fwdSpread, kMod, timeToExpiry, vol, true);
      double iv;
      try {
        iv = BlackFormulaRepository.impliedVolatility(p3 / fwdAnnuity, fwdSpread, kMod, timeToExpiry, true);
      } catch (final IllegalArgumentException e) {
        iv = 0;
      }

      System.out.println(k * TEN_THOUSAND + "\t" + p + "\t" + p2 + "\t" + p3 + "\t" + p4 + "\t" + iv);
    }
  ...
  public void portfolioAdjustmentTest() {
    ...
    for (int i = 0; i < nIndexTerms; i++) {
      final double intrPUF = INDEX_CAL.indexPV(pillarCDX[i], indexCoupon, yieldCurve, intrCreditCurves, intrRR);
    ...
    final ISDACompliantCreditCurve[] adjCreditCurves = PSA.adjustCurves(pillarCDX, indexCoupon, yieldCurve, intrCreditCurves, intrRR, puf);
    //check the adjustment worked 
    for (int i = 0; i < nIndexTerms; i++) {
      final double adjPUF = INDEX_CAL.indexPV(pillarCDX[i], indexCoupon, yieldCurve, adjCreditCurves, intrRR);
    ...
    for (final String name : exampleNames) {
      ...
      for (final CDSAnalytic cds : exampleCDS) {
        final double parSpread = PRICER.parSpread(cds.withRecoveryRate(rr), yieldCurve, adjCreditCurves[pos]);
        System.out.print("\t" + parSpread * TEN_THOUSAND);
      }
      System.out.print("\n");
    }
