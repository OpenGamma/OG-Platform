...
import com.opengamma.analytics.financial.provider.description.SABRSwaptionProvider;
...
public class SwaptionPhysicalFixedIborLMMDDMethodTest {
  ...
  private static final ParameterSensitivityParameterCalculator<LiborMarketModelDisplacedDiffusionProviderInterface> PS_LL_C = new ParameterSensitivityParameterCalculator<LiborMarketModelDisplacedDiffusionProviderInterface>(
      PVCSLLC);
  ...
  private static final SABRInterestRateParameters SABR_PARMETERS = TestsDataSetsSABR.createSABR1();
  ...
  public void performance() {
    ...
    for (int looptest = 0; looptest < nbTest; looptest++) {
      ...
      swapDefinition[looptest] = SwapFixedIborDefinition.from(SETTLEMENT_DATE, SWAP_TENOR, EUR1YEURIBOR6M, NOTIONAL, rate[looptest], FIXED_IS_PAYER);
    ...
    for (int looptest = 0; looptest < nbTest; looptest++) {
      pvPayerLongApproximation[looptest] = METHOD_LMM.presentValue(swaption[looptest], LMM_MULTICURVES);
    }
    ...
    startTime = System.currentTimeMillis();
    for (int looptest = 0; looptest < nbTest; looptest++) {
      pvLmmSensi[looptest] = METHOD_LMM.presentValueLMMSensitivity(swaption[looptest], LMM_MULTICURVES);
    }
    endTime = System.currentTimeMillis();
    ...
    for (int looptest = 0; looptest < nbTest2; looptest++) {
      pvMC[looptest] = METHOD_LMM_MC.presentValue(swaption[looptest], EUR, LMM_MULTICURVES);
    }
    endTime = System.currentTimeMillis();
  ...
  public void calibrateExactPriceAmortized() {
    ...
    final SuccessiveRootFinderCalibrationEngine<SABRSwaptionProviderInterface> calibrationEngineShift = new SuccessiveRootFinderLMMDDCalibrationEngine<SABRSwaptionProviderInterface>(objectiveShift);
