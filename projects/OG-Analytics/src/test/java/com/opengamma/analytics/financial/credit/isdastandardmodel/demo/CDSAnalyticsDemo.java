...
public class CDSAnalyticsDemo extends ISDABaseTest {
  ...
  private static final double[] RATES = new double[] {0.001515, 0.001965, 0.002346, 0.003268, 0.005451, 0.005875, 0.010035, 0.013935, 0.017125, 0.01973, 0.021855, 0.02364, 0.025135, 0.02638,
  ...
  private static final LocalDate[] TEST_DATES = new LocalDate[] {LocalDate.of(2014, 7, 2), LocalDate.of(2014, 7, 10), LocalDate.of(2014, 7, 30), LocalDate.of(2014, 9, 1), LocalDate.of(2014, 10, 1),
  ...
  public void upfrontModelDemo() {
    ...
    final ISDACompliantCreditCurve fittedConstCreditCurve = creditCurveBuilder.calibrateCreditCurve(cds, coupon, YIELD_CURVE, puf);
    ...
    for (int i = 0; i < 21; i++) {
      ...
      final QuotedSpread spreadQuote = converter.convert(cds, pufQuote, YIELD_CURVE);
      System.out.format("PUF: %.4f%%, Quoted Spread: %.3fbps\n", pufQuote.getPointsUpFront() * ONE_HUNDRED, spreadQuote.getQuotedSpread() * TEN_THOUSAND);
    }
  ...
  public void cashSettlementDemo() {
    ...
    final double principle2 = notional * pricer.pv(cds, YIELD_CURVE, fittedConstCreditCurve, coupon, PriceType.CLEAN);
    final double cashSettlement2 = notional * pricer.pv(cds, YIELD_CURVE, fittedConstCreditCurve, coupon, PriceType.DIRTY);
    final double marketValue2 = notional * pricer.pv(cds, YIELD_CURVE, fittedConstCreditCurve, coupon, PriceType.DIRTY, 0.0);
  ...
  public void mixedQuoteCurveBuildDemo() {
    ...
    quotes[0] = new ParSpread(80 * ONE_BP);
    quotes[1] = new PointsUpFront(coupon, -0.3 * ONE_PC);
    quotes[2] = new PointsUpFront(coupon, 0.5 * ONE_PC);
    quotes[3] = new QuotedSpread(coupon, 130 * ONE_BP);
    quotes[4] = new ParSpread(150 * ONE_BP);
    quotes[5] = new QuotedSpread(coupon, 225 * ONE_BP);
  ...
  public void cs01Demo() {
    ...
    final CDSQuoteConvention quotedSpread = new QuotedSpread(coupon, tradeLevel);
    final double notional = 1e7; //10MM
    ...
    final MarketQuoteConverter converter = new MarketQuoteConverter();
    final PointsUpFront puf = converter.convert(cds5Y, (QuotedSpread) quotedSpread, YIELD_CURVE);
