...
public class EquityIndexFutureOptionTest {
  ...
  private static final EquityIndexFutureOption AMERICAN_PUT = new EquityIndexFutureOption(EXPIRY, UNDERLYING, STRIKE, EXERCISE, IS_CALL);
  ...
  public void testObject() {
    ...
    assertEquals(POINT_VALUE, AMERICAN_PUT.getUnderlying().getUnitAmount());
    ...
    EquityIndexFutureOption other = new EquityIndexFutureOption(EXPIRY, UNDERLYING, STRIKE, EXERCISE, IS_CALL);
    assertEquals(AMERICAN_PUT, other);
    assertEquals(AMERICAN_PUT.hashCode(), other.hashCode());
    other = new EquityIndexFutureOption(EXPIRY + 0.0001, UNDERLYING, STRIKE, EXERCISE, IS_CALL);
    assertFalse(AMERICAN_PUT.equals(other));
    other = new EquityIndexFutureOption(EXPIRY, new EquityIndexFuture(EXPIRY, SETTLEMENT, STRIKE, CCY, POINT_VALUE + 1), STRIKE, EXERCISE, IS_CALL);
    assertFalse(AMERICAN_PUT.equals(other));
    other = new EquityIndexFutureOption(EXPIRY, UNDERLYING, STRIKE + 1, EXERCISE, IS_CALL);
    assertFalse(AMERICAN_PUT.equals(other));
    other = new EquityIndexFutureOption(EXPIRY, UNDERLYING, STRIKE, ExerciseDecisionType.EUROPEAN, !IS_CALL);
    assertFalse(AMERICAN_PUT.equals(other));
  }
