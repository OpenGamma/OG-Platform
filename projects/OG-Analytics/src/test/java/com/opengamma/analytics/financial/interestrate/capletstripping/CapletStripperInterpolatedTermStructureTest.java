...
public class CapletStripperInterpolatedTermStructureTest extends CapletStrippingSetup {
  ...
  public void atmTest() {
    ...
    DoubleMatrix1D expFitParms = new DoubleMatrix1D(0.0750077681018984, -0.0417132310902179, 0.443334588249104,
        0.166354129225537, -0.0955443232821012, -0.535813313851482, -0.81400903918072);
    double expChiSqr = 0.0;

    testStripping(stripper, getATMCapPrices(), MarketDataType.PRICE, expChiSqr, expFitParms, 1e-14, false);
  }
  ...
  public void singleStrikeTest() {
    double[][] expFitParms = new double[][] {
      {0.07519558925857502, 0.06099595006987337, 0.37887791113990626, 0.2900814932812122, 0.06960865030557412,
        -0.05342521479925498 },
      {0.2888421248987955, -0.0023192971048977065, -0.15913446980405835, -0.2962382393117878, -0.4773062729997372 },
      {-0.17164504785623153, 0.17570630956403765, -0.09852806432927508, -0.23506962123817332, -0.41048624505277914,
        -0.6301356867782051 },
      {0.3908797227276975, -0.3238072319276149, -0.5393466581064617, -0.7213032534286311 },
      {-0.7283917245109782, 0.2553434284914208, -0.2378442519094908, -0.3704381964960703, -0.536862242806576,
        -0.8058029253911454 },
      {0.827445095514852, -0.4198900590695616, -0.6269795252250308, -0.8875937407303798 },
      {0.9532565315179226, 0.2851870481750914, 0.23441102409602055, -0.16207126482856957, -0.652262655104053,
        -0.9099696638475132 },
      {0.9371181616741376, 0.34005473961956495, 0.23237903957042214, -0.19618930329921386, -0.40683572799405837,
        -0.6596470520871216, -0.9613088609863392 },
      {0.7059889604800385, 0.45927089229020324, 0.11425056866252968, -0.36451987986750245 },
      {0.955999407622807, 0.40220172717867936, 0.19913928224094274, -0.15816695045653018, -0.4590787700861603,
        -0.6932984278278611, -0.9875408880086803 },
      {0.9448245261357552, 0.41772275182543434, 0.19395773066035696, -0.15365694720478604, -0.4229538051369399,
        -0.6989629404321959, -0.997435029791332 },
      {0.9644014702150517, 0.4533491035091301, 0.1699873426536261, -0.16922659200325504, -0.42043388566407425,
        -0.6892676775943389, -1.0090832784881916 },
      {0.6984521044968122, 0.5556741207844491, -0.1921052130631996, -0.39739908140320057, -0.684338560893483,
        -1.016715992733998 },
      {1.1191378168668937, -0.18251768035905208, -0.3962744821361997, -0.6884914142373438, -0.9972361085539848 },
      {0.5838225819129124, 0.17592675122488316, -0.16435508774233396, -0.4081223090812207, -0.6539226486174113,
        -0.9615281610297096 },
      {0.5857751937847466, 0.18152902949991212, -0.16919316844578738, -0.39593555754486615, -0.6440943082426583,
        -0.9374060706820373 },
    ...
    for (int i = 0; i < n; i++) {
      ...
      testStripping(strippers[i], getCapPrices(i), MarketDataType.PRICE, 0.0, new DoubleMatrix1D(expFitParms[i]),
          1e-14, false);
    }
    ...
    for (int i = 0; i < n; i++) {
      testStripping(strippers[i], getCapVols(i), MarketDataType.VOL, 0.0, new DoubleMatrix1D(expFitParms[i]), 1e-4,
          false);
    }
  }
  ...
  public void randomStartTest() {
    ...
    DoubleMatrix1D expVolFitParms = new DoubleMatrix1D(0.0751955883987435, 0.0609959501482534, 0.378877911140902,
        0.290081493287973, 0.0696086502994876, -0.0534252147847549);
    DoubleMatrix1D expPriceFitParms = new DoubleMatrix1D(0.075195589258575, 0.0609959500698733, 0.378877911139906,
        0.290081493281212, 0.0696086503055741, -0.0534252147992549);
  ...
  public void globalFitTest() {
    ...
    double expPriceChiSq = 238653.409503741;
    DoubleMatrix1D expPriceFitParms = new DoubleMatrix1D(-4.15580547109701, -0.304212974580643, 0.172831987560697,
        0.198455163737413, -0.135298615671123, -0.565282388889337, -0.9691261199302, -0.947227800414312);
    testStripping(stripper, prices, MarketDataType.PRICE, vega, expPriceChiSq, expPriceFitParms, 1e-14, false);

    // fit to vols
    double expVolChiSq = 242619.18324317678;
    DoubleMatrix1D expVolFitParms = new DoubleMatrix1D(-4.30213860658499, -0.317785074052976, 0.171692755804742,
        0.202209112938613, -0.134975254807032, -0.554874332703476, -0.973564599389498, -0.917346420590909);
    testStripping(stripper, vols, MarketDataType.VOL, errors, expVolChiSq, expVolFitParms, 1e-14, false);

    // guess a start val
    DoubleMatrix1D guess = new DoubleMatrix1D(knots.length, 0.3);
    testStripping(stripper, prices, MarketDataType.PRICE, vega, guess, expPriceChiSq, expPriceFitParms, 5e-5, false);
    testStripping(stripper, vols, MarketDataType.VOL, errors, guess, expVolChiSq, expVolFitParms, 1e-4, false);
  }
