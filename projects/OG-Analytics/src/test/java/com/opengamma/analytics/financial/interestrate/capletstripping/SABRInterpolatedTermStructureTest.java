...
public class SABRInterpolatedTermStructureTest extends CapletStrippingSetup {
  ...
  public void test() {
    ...
    VectorFunction alpha = alphaPro.from(t);
    VectorFunction beta = betaPro.from(t);
    VectorFunction rho = rhoPro.from(t);
    VectorFunction nu = nuPro.from(t);

    VectorFunction modelToSmileParms = new ConcatenatedVectorFunction(new VectorFunction[] {alpha, beta, rho, nu });

    double[] errors = new double[caps.size()];
    Arrays.fill(errors, 0.0001); // 1bps
    CapletStripper stripper = new CapletStripperSmileModel<SABRFormulaData>(pricer, new SABRHaganVolatilityFunction(),
        modelToSmileParms);
