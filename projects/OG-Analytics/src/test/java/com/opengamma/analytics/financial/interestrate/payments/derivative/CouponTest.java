...
public class CouponTest {
  ...
  public void testObject() {
    ...
    Coupon other = new MyCoupon(CCY, PAYMENT_TIME, ACCRUAL_TIME, NOTIONAL);
    assertEquals(COUPON, other);
    assertEquals(COUPON.hashCode(), other.hashCode());
    assertEquals(COUPON.toString(), "Currency=AUD, payment time=0.5, year fraction = 0.51, notional = 100.0");
    other = new MyCoupon(Currency.CAD, PAYMENT_TIME, ACCRUAL_TIME, NOTIONAL);
    assertFalse(other.equals(COUPON));
    other = new MyCoupon(CCY, PAYMENT_TIME + 1, ACCRUAL_TIME, NOTIONAL);
    assertFalse(other.equals(COUPON));
    other = new MyCoupon(CCY, PAYMENT_TIME, ACCRUAL_TIME + 1, NOTIONAL);
    assertFalse(other.equals(COUPON));
    other = new MyCoupon(CCY, PAYMENT_TIME, ACCRUAL_TIME, NOTIONAL + 1);
    assertFalse(other.equals(COUPON));
  }

  private static class MyCoupon extends Coupon {

    public MyCoupon(final Currency currency, final double paymentTime, final double paymentYearFraction, final double notional) {
