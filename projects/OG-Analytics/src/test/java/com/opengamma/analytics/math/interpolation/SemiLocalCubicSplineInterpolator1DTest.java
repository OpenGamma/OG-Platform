...
public class SemiLocalCubicSplineInterpolator1DTest {
  ...
  //  @Test
  //  public void toBeModifiedFunctionTest() {
  //    final int nData = 10;
  //    final double[] xValues = new double[nData];
  ...
  //      final double ref2 = resPrim2[i];
  //      final double ref3 = resPrim3[i];
  //      assertEquals(ref1, INTERP1D.interpolate(dataBund1, xKeys[i]), 1.e-14 * Math.max(Math.abs(ref1), 1.));
  //      assertEquals(ref2, INTERP1D.interpolate(dataBund2, xKeys[i]), 1.e-14 * Math.max(Math.abs(ref2), 1.));
  //      assertEquals(ref3, INTERP1D.interpolate(dataBund3, xKeys[i]), 1.e-14 * Math.max(Math.abs(ref3), 1.));
  //    }
  //
  //    for (int j = 0; j < nData; ++j) {
  //      yValues1Up[j] = yValues1[j] * (1. + EPS);
