...
public class CDSPaperExamples extends ISDABaseTest {
  ...
  public void yieldCurveDump() {
    ...
    for (int i = 0; i < n; i++) {
      System.out.println(YIELD_CURVE_POINTS[i] + "\t" + YIELD_CURVE_RATES[i] + "\t" + YIELD_CURVE.getTimeAtIndex(i) + "\t" + YIELD_CURVE.getZeroRateAtIndex(i));
    }
  }
  ...
  public void creditCurveDump() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      System.out.println(TENORS[i].toString() + "\t" + PILLAR_DATES[i].toString(DATE_FORMAT) + "\t" + SPREADS[i] * TEN_THOUSAND + "\t" + t + "\t" + CREDIT_CURVE.getSurvivalProbability(t));
    }
  }
  ...
  public void pufVsQuotedSpread() {
    ...
    for (int i = 0; i < 100; i++) {
      double qs = i * 10;
      double puf0 = PUF_CONVERTER.convert(cds, new QuotedSpread(0.0, qs * ONE_BP), YIELD_CURVE).getPointsUpFront();
      double puf100 = PUF_CONVERTER.convert(cds, new QuotedSpread(0.01, qs * ONE_BP), YIELD_CURVE).getPointsUpFront();
      double puf500 = PUF_CONVERTER.convert(cds, new QuotedSpread(0.05, qs * ONE_BP), YIELD_CURVE).getPointsUpFront();
  ...
  public void parRateSensitivityTest() {
    ...
    for (int i = 0; i < IMM_DATES.length; i = i + 2) {
      ...
      System.out.print(mat.toString(DATE_FORMAT));
  ...
  public void analyticCS01test() {
    ...
    double[][] analCS01 = ANALYTIC_SPREAD_SENSE_CAL.bucketedCS01FromCreditCurve(cds, coupons, PILLAR_CDSS, YIELD_CURVE, CREDIT_CURVE);
    ...
    for (int i = 0; i < nMat; i++) {
      rowHeadings[i] = MATURITIES_6M_STEP[i].toString(DATE_FORMAT);
  ...
  void analyticVFDCS01Test() {
    ...
    CDSAnalytic cds = CDS_FACTORY.makeCDS(TRADE_DATE, STARTDATE, mat);
    double[] analCS01 = ANALYTIC_SPREAD_SENSE_CAL.bucketedCS01FromCreditCurve(cds, COUPON, PILLAR_CDSS, YIELD_CURVE, CREDIT_CURVE);
    double pCS01 = FD_SPREAD_SENSE_CAL.parallelCS01FromParSpreads(cds, COUPON, YIELD_CURVE, PILLAR_CDSS, SPREADS, ONE_BP, BumpType.ADDITIVE);
  ...
  public void spreadHedgeDemo() {
    ...
    Arrays.fill(coupons, COUPON);
    double[][] temp = ANALYTIC_SPREAD_SENSE_CAL.bucketedCS01FromCreditCurve(PILLAR_CDSS, coupons, PILLAR_CDSS, YIELD_CURVE, CREDIT_CURVE);
  ...
  public void bucketedCS01FromFlatSpreadDemo() {
    ...
    for (int i = 0; i < nMat; i++) {
      ...
      double[] bCS01 = FD_SPREAD_SENSE_CAL.bucketedCS01FromParSpreads(cds[i], COUPON, YIELD_CURVE, PILLAR_CDSS, spreads, ONE_BP, BumpType.ADDITIVE);
      double pCS01 = FD_SPREAD_SENSE_CAL.parallelCS01FromParSpreads(cds[i], COUPON, YIELD_CURVE, PILLAR_CDSS, spreads, ONE_BP, BumpType.ADDITIVE);

      System.out.print(MATURITIES_1Y_STEP[i].toString(DATE_FORMAT));
      double sum = 0;
      for (int j = 0; j < nPillars; j++) {
        sum += bCS01[j];
  ...
  public void yieldSenseTest() {
    ...
    double[] rates = new double[] {0.00445, 0.009488, 0.012337, 0.017762, 0.01935, 0.020838, 0.01652, 0.02018, 0.023033, 0.02525, 0.02696, 0.02825, 0.02931, 0.03017 };
    ISDACompliantYieldCurve yc = makeYieldCurve(TRADE_DATE, SPOT_DATE, ycPoints, instuments, rates, ACT360, D30360, Period.ofYears(1));
