...
public class MonotoneConvexSplineInterpolatorTest {
  ...
  public void knotsControlTest1() {
    ...
    for (int i = 1; i < nData - 1; ++i) {
      ...
      assertEquals(values.getData()[i], yValues[i], ref * EPS);
    }
    {
      final double ref = yValues[1] == 0. ? 1. : Math.abs(yValues[1]);
      assertEquals(interpolator.interpolate(xValues, yValues, xValues[1]), yValues[1], ref * EPS * 10);
    }
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        final double ref = modifiedFwdsMatrix[i][j] == 0. ? 1. : Math.abs(modifiedFwdsMatrix[i][j]);
        assertEquals(valuesMatrix.getData()[i][j], yValuesMatrix[i][j], ref * EPS * 10);
      }
    }
