...
public class CurveCalibrationTest extends ISDABaseTest {
  private static final PointsUpFrontConverter PUF_CONVERTED = new PointsUpFrontConverter();
  ...
  static {
    ...
    for (int i = 0; i < n; i++) {
      ...
      CURVE_CDS[i] = new CDSAnalytic(TRADE_DATE, EFFECTIVE_DATE, CASH_SETTLE_DATE, STARTDATE, MATURITIES[i], PROCTECTION_START, TENOR, STUB, PAY_ACC_ON_DEFAULT, RECOVERY_RATE);
    }
  ...
  public void test() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      parSpread[i] = new ParSpread(PAR_SPREADS[i]);
    }
