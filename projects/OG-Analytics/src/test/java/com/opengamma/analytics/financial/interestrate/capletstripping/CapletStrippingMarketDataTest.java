...
public class CapletStrippingMarketDataTest {
  ...
  static {
    ...
    for (int i = 0; i < CAP_ATM_VOL.length; i++) {
      CapFloor cap = SimpleCapFloorMaker.makeCap(CUR, INDEX, 0, FREQUENCY * CAP_EXPIRIES[i], "funding", "3m Libor", 0.0, true);
    ...
    int start = 1; // miss out first (known) libor rate
    for (int i = 0; i < CAP_EXPIRIES.length; i++) {
      final int end = FREQUENCY * CAP_EXPIRIES[i];
      for (int j = 0; j < STRIKES[i].length; j++) {
        CapFloor cap = SimpleCapFloorMaker.makeCap(CUR, INDEX, start, end, "funding", "3m Libor", STRIKES[i][j], true);
        CAPS.add(cap);
    ...
    for (final String name : NAMES) {
      if (name == "beta") {
        ...
        Interpolator1D flat = CombinedInterpolatorExtrapolatorFactory.getInterpolator(Interpolator1DFactory.LINEAR, Interpolator1DFactory.FLAT_EXTRAPOLATOR);
  ...
  public void testFwd() {
    ...
    VolatilityModel1D volModel = new FlatVolatility(vol);
    ...
    double price = pricer.price(volModel);
    double impVol = pricer.impliedVol(volModel);
    System.out.println(fwd + "\t" + impVol + "\t" + price);

    cap = SimpleCapFloorMaker.makeCap(CUR, INDEX, start, end, "funding", "3m Libor", 0.03, true);
  ...
  public void testStripping() {
    ...
    DoubleMatrix1D vStart = new DoubleMatrix1D(start);
    ...
    SABRTermStructureParameters sabrTS = (SABRTermStructureParameters) VOL_MODEL_PROVIDER.evaluate(lsRes.getFitParameters());
    // print the SABR curves
    final int nPoints = 101;
    System.out.println("t\talpha\tbeta\trho\tnu");
    for (int i = 0; i < nPoints; i++) {
      double t = i * 10.0 / (nPoints - 1);
    ...
    for (int i = 0; i < CAP_EXPIRIES.length; i++) {
      final int end = FREQUENCY * CAP_EXPIRIES[i];
      System.out.println(CAP_EXPIRIES[i] + " year cap");
      for (int j = 0; j < nStrikes; j++) {
        double k = 0.001 + j * 0.1 / (nStrikes - 1);
        CapFloor cap = SimpleCapFloorMaker.makeCap(CUR, INDEX, 1, end, "funding", "3m Libor", k, true);
        CapFloorPricer pricer = new CapFloorPricer(cap, YIELD_CURVES);
        double vol = pricer.impliedVol(sabrTS);
