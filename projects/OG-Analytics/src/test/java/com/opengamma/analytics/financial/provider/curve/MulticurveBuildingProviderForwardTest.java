...
import com.opengamma.analytics.financial.provider.calculator.LastFixingTimeCalculator;
 ...
 * @author marc
 */
public class MulticurveBuildingProviderForwardTest {

  private static final Interpolator1D INTERPOLATOR_LINEAR = CombinedInterpolatorExtrapolatorFactory.getInterpolator(Interpolator1DFactory.LINEAR, Interpolator1DFactory.FLAT_EXTRAPOLATOR,
      Interpolator1DFactory.FLAT_EXTRAPOLATOR);

  private static final LastFixingTimeCalculator LAST_FIXING_CALCULATOR = LastFixingTimeCalculator.getInstance();
  ...
  public static final GeneratorInstrument[] FWD3_USD_GENERATORS = new GeneratorInstrument[] {USD6MLIBOR3M, USD6MLIBOR3M, USD6MLIBOR3M, USD6MLIBOR3M, USD6MLIBOR3M, USD6MLIBOR3M, USD6MLIBOR3M};
  /** Tenors for the Fwd 3M USD curve */
  public static final Period[] FWD3_USD_TENOR = new Period[] {Period.ofMonths(6), Period.ofYears(1), Period.ofYears(2), Period.ofYears(3), Period.ofYears(5), Period.ofYears(7), Period.ofYears(10)};
  ...
  static {
    ...
    GeneratorYDCurve genIntLinMat = new GeneratorCurveYieldInterpolated(MATURITY_CALCULATOR, INTERPOLATOR_LINEAR);
    GeneratorYDCurve genIntLinFix = new GeneratorCurveYieldInterpolated(LAST_FIXING_CALCULATOR, INTERPOLATOR_LINEAR);
  ...
  private static final ProviderForwardBuildingRepository CURVE_BUILDING_REPOSITORY = new ProviderForwardBuildingRepository(TOLERANCE_ROOT, TOLERANCE_ROOT, STEP_MAX);
  ...
  public void curveConstructionTest(final InstrumentDefinition<?>[][][] definitions, final MulticurveProviderForward curves, final boolean withToday, int block) {
  ...
  public void forwardAnalysis() {
    MulticurveProviderInterface marketFwd = CURVES_PAR_SPREAD_MQ_WITHOUT_TODAY_BLOCK.get(0).getFirst();
    int jump = 1;
    int startIndex = 0;
    int nbDate = 2750;
    ZonedDateTime startDate = ScheduleCalculator.getAdjustedDate(NOW, USDLIBOR3M.getSpotLag() + startIndex * jump, NYC);
    double[] rateFwd = new double[nbDate];
    double[] startTime = new double[nbDate];
    try {
      FileWriter writer = new FileWriter("fwd-fwd.csv");
  ...
  private static Pair<MulticurveProviderForward, CurveBuildingBlockBundle> makeCurvesFromDefinitions(final InstrumentDefinition<?>[][][] definitions, GeneratorYDCurve[][] curveGenerators,
      String[][] curveNames, MulticurveProviderForward knownData, final AbstractInstrumentDerivativeVisitor<MulticurveProviderInterface, Double> calculator,
