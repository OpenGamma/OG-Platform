...
public class CapFloorCMSSpreadSABRBinormalMethod implements PricingMethod {
   ...
   * ??? The change of ATM strike due to curve change is not taken into account in the curve sensitivity;
  ...
  public InterestRateCurveSensitivity presentValueCurveSensitivity(final CapFloorCMSSpread cmsSpread, final SABRInterestRateDataBundle sabrData) {
    ...
    CouponCMS cmsCoupon2 = CouponCMS.from(cmsSpread, cmsSpread.getUnderlyingSwap2(), cmsSpread.getSettlementTime());
    cmsCoupon2 = cmsCoupon2.withNotional(Math.abs(cmsCoupon2.getNotional()));
    CapFloorCMS cmsCap1 = CapFloorCMS.from(cmsCoupon1, forward1, true); // ATM forward cap CMS
    CapFloorCMS cmsCap2 = CapFloorCMS.from(cmsCoupon2, forward2, true); // ATM forward cap CMS
    ...
    NORMAL_PRICE.getPriceAdjoint(optionSpread, dataSpread, cmsSpreadPvDerivative);
    // Backward sweep
    ...
    double expectation2Bar = -cmsSpreadPvDerivative[0] * cmsSpreadPvBar + -cmsCap2PriceNormalDerivative[0] / cmsCap2PriceNormalDerivative[1] * cmsCap2ImpliedVolatilityBar;
    ...
    double expectation1Bar = cmsSpreadPvDerivative[0] * cmsSpreadPvBar + -cmsCap1PriceNormalDerivative[0] / cmsCap1PriceNormalDerivative[1] * cmsCap1ImpliedVolatilityBar;
    double factorBar = -cmsCoupon1Pv / (factor * factor) * expectation1Bar + -cmsCoupon2Pv / (factor * factor) * expectation2Bar - cmsCap2Pv / factor / cmsCap2PriceNormalDerivative[1]
        * cmsCap2ImpliedVolatilityBar - cmsCap1Pv / factor / cmsCap1PriceNormalDerivative[1] * cmsCap1ImpliedVolatilityBar;
    double discountFactorPaymentBar = cmsCap1.getNotional() * cmsCap1.getPaymentYearFraction() * factorBar;
    double cmsCap2PvBar = 1.0 / cmsCap2PriceNormalDerivative[1] * cmsCap2ImpliedVolatilityBar;
    double cmsCap1PvBar = 1.0 / cmsCap1PriceNormalDerivative[1] * cmsCap1ImpliedVolatilityBar;
    double cmsCoupon2PvBar = expectation2Bar / factor;
    double cmsCoupon1PvBar = expectation1Bar / factor;
    //Calibration strike dependency -- START
    double forward1Bar = -cmsCap1PriceNormalDerivative[2] / cmsCap1PriceNormalDerivative[1] * cmsCap1ImpliedVolatilityBar;
    double forward2Bar = -cmsCap2PriceNormalDerivative[2] / cmsCap2PriceNormalDerivative[1] * cmsCap2ImpliedVolatilityBar;
    forward1Bar += METHOD_CMS_CAP.presentValueStrikeSensitivity(cmsCap1, sabrData) * cmsCap1PvBar;
    forward2Bar += METHOD_CMS_CAP.presentValueStrikeSensitivity(cmsCap2, sabrData) * cmsCap2PvBar;
