 ...
 */
public final class VarianceSwapRatesSensitivityCalculator2 {

  private static final VarianceSwapStaticReplicationFactory FACTORY = new VarianceSwapStaticReplicationFactory();
  private static final VarianceSwapRatesSensitivityCalculator2 INSTANCE = new VarianceSwapRatesSensitivityCalculator2();

  public static VarianceSwapRatesSensitivityCalculator2 getInstance() {
    return INSTANCE;
  }

  private VarianceSwapRatesSensitivityCalculator2() {
  }
  ...
  public Double calcForwardSensitivity(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market, final double relShift) {
    Validate.notNull(swap, "null VarianceSwap");
    Validate.notNull(market, "null VarianceSwapDataBundle");

    VarianceSwapPresentValueCalculator2 pricer = VarianceSwapPresentValueCalculator2.getInstance();

    // Shift UP
    VarianceSwapDataBundle2<?> bumpedMarket = new VarianceSwapDataBundle2(market.getVolatilitySurface(), market.getDiscountCurve(), market.getForwardCurve().withFractionalShift(relShift));
    final double pvUp = pricer.visitVarianceSwap(swap, bumpedMarket);

    // Shift Down
    bumpedMarket = new VarianceSwapDataBundle2(market.getVolatilitySurface(), market.getDiscountCurve(), market.getForwardCurve().withFractionalShift(-relShift));
   ...
   * @return A Double. Currency amount per unit amount change in the black volatility
   */
  public Double calcForwardSensitivity(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
   ...
   */
  @SuppressWarnings({"unchecked", "rawtypes" })
  public Double calcDiscountRateSensitivity(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market, final double shift) {
   ...
   * @return A Double in the currency, deriv.getCurrency(). Currency amount per unit amount change in discount rate
   */
  public Double calcDiscountRateSensitivity(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
    final double relativeShift = 0.01;
   ...
   */
  public Double calcPV01(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
   ...
   */
  public DoubleMatrix1D calcDeltaBucketed(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
    Validate.notNull(swap, "null VarianceSwap");
    Validate.notNull(market, "null VarianceSwapDataBundle");
   ...
   * @return A Double. Currency amount per unit amount change in the black volatility
   */
  public Double calcBlackVegaParallel(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
  ...
  public Double calcBlackVegaParallel(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market, final double shift) {
    Validate.notNull(swap, "null VarianceSwap");
    Validate.notNull(market, "null VarianceSwapDataBundle");
    ...
    double pvUp = pricer.visitVarianceSwap(swap, new VarianceSwapDataBundle2(upSurface, market.getDiscountCurve(), market.getForwardCurve()));

    // Parallel shift DOWN
    BlackVolatilitySurface<?> downSurface = market.getVolatilitySurface().withShift(-shift, true);
    double pvDown = pricer.visitVarianceSwap(swap, new VarianceSwapDataBundle2(downSurface, market.getDiscountCurve(), market.getForwardCurve()));
   ...
   */
  public NodalDoublesSurface calcBlackVegaForEntireSurface(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
    final double shift = 0.001; // Shift each vol point by what? +/- 0.1%
   ...
   */
  public NodalDoublesSurface calcBlackVegaForEntireSurface(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market, final double shift) {
    Validate.notNull(swap, "null VarianceSwap");
    Validate.notNull(market, "null VarianceSwapDataBundle");
   ...
   */
  @SuppressWarnings({"rawtypes", "unchecked" })
  public double calcBlackVegaForSinglePoint(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market, double maturity, double strike, final double shift) {

    final VarianceSwapPresentValueCalculator2 pricer = VarianceSwapPresentValueCalculator2.getInstance();
    ...
    VarianceSwapDataBundle2 bumpedMarket = new VarianceSwapDataBundle2(market.getVolatilitySurface().withSurface(bumpedVolUp), market.getDiscountCurve(), market.getForwardCurve());
    final double pvUp = pricer.visitVarianceSwap(swap, bumpedMarket);

    // shift DOWN
    final InterpolatedDoublesSurface bumpedVolDown = volShifter.evaluate(blackSurf, maturity, strike, -shift);
    bumpedMarket = new VarianceSwapDataBundle2(market.getVolatilitySurface().withSurface(bumpedVolDown), market.getDiscountCurve(), market.getForwardCurve());
    final double pvDown = pricer.visitVarianceSwap(swap, bumpedMarket);
   ...
   */
  public Double calcSensitivityToFairVariance(final VarianceSwap swap, final VarianceSwapDataBundle2<?> market) {
    Validate.notNull(swap, "null VarianceSwap");
    Validate.notNull(market, "null VarianceSwapDataBundle");
