  ...
  public LeastSquareResults solve(final double[] strikes, final double[] blackVols, final double[] errors, final double[] initialValues, final BitSet fixed) {
    ...
    final ParameterizedFunction<Double, DoubleMatrix1D, Double> function = new ParameterizedFunction<Double, DoubleMatrix1D, Double>() {
      ...
      public Double evaluate(final Double strike, final DoubleMatrix1D fp) {
        ...
        final SVIFormulaData data = new SVIFormulaData(a, b, rho, sigma, m);
        return FORMULA.getVolatilityFunction(new EuropeanVanillaOption(strike, 0, true)).evaluate(data);
      }
    };
