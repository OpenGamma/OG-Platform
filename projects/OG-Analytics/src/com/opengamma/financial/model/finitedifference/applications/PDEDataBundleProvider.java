...
public class PDEDataBundleProvider {
   ...
   *  {@latex.inline $\\frac{\\partial V}{\\partial \\tau} - \\frac{\\sigma^2 s^2}{2} \\frac{\\partial^2 V}{\\partial s^2} -rs \\frac{\\partial V}{\\partial s} + rV = 0$}
   *  where the 'time' term  {@latex.inline $\\tau$} is time to maturity, so the initial state is at {@latex.inline $\\tau = 0$} is a standard option  payoff
  ...
  /**
   * Set up a Black-Scholes PDE where the space variable is the log of the spot  {@latex.inline $x=\\ln(s)$}
   * {@latex.inline $\\frac{\\partial V}{\\partial \\tau} - \\frac{\\sigma^2}{2} \\frac{\\partial^2 V}{\\partial x^2} -(\\frac{\\sigma^2}{2}+r) \\frac{\\partial V}{\\partial x} + rV = 0$}
   * @param vol  The volatility
   * @param rate The rate
   * @param strike The strike
   * @param isCall is a call
  ...
  /**
   * Set up a backwardsPDE for a model where the underlying follows the SDE {@latex.inline $ ds = \\mu(t)sdt + \\sigma(t,s) s^{\\beta} dW$}, with local volatility 
   *  {@latex.inline $\\sigma(t,s)$}. The resulting  PDE is {@latex.inline $\\frac{\\partial V}{\\partial \\tau} - \\sigma(t,s)\\frac{s^{2\\beta}}{2} \\frac{\\partial^2 V}{\\partial s^2} 
   *  -\\mu(t)s \\frac{\\partial V}{\\partial s} + \\mu(t) V = 0$} <p>
   *  As usual with the backwards PDEs, 'time' is time-to-maturity ({@latex.inline $\\tau$}) and thus starts at zero (i.e. at maturity). The initial condition is a European option payoff 
   *  (so {@latex.inline $V(0,s) = (\\omega(s-k))^+$, where is +1 for call and -1 for put}). Real (i.e. calendar) time, t, appears in the local volatility and drift terms with 
   *  {@latex.inline $t = T - \\tau$} where T is the maturity. The solution at  {@latex.inline $t = 0 (\\tau=T)$} is the option price for a particular spot. 
   * @param forward The forward curve TODO the assumption here is that the drift  {@latex.inline $\\mu(t)$} is the same as the short rate used for discounting (both of which are taken off
   ...
   *  {@latex.inline $\\sigma(t,s)$} and level  {@latex.inline $\\sigma_i $} which takes on one of two values depending on the state of the continuous 2-state hidden Markov chain. The 
   *  resulting pair of PDEs is {@latex.inline $\\frac{\\partial V_1}{\\partial \\tau} - \\frac{\\sigma_1^2 s^{2\\beta}}{2} \\frac{\\partial^2 V_1}{\\partial s^2} 
   ...
   * + (\\mu(t) + \\lambda_{21})V_2 - \\lambda_{21} V_1= 0 $} <p>
   *  As usual with the backwards PDEs, 'time' is time-to-maturity ({@latex.inline $\\tau$}) and thus starts at zero (i.e. at maturity). The initial condition is a call payoff (so 
   *  {@latex.inline $V_1(0,s) = V_2(0,s) = (s-k)^+$}). Real (i.e. calendar) time, t, appears in the drift term with {@latex.inline $t = T - \\tau$} where T
   *  is the maturity. The solution at  {@latex.inline $t = 0 (\\tau=T)$} for   {@latex.inline $V_1$ and $V_2$} is the option price if the Markov chain started in state 1 or 2. If the 
  ...
  /**
   * Set up a backwards coupled PDE for a model where the underlying follows the SDE {@latex.inline $ ds = \\mu(t)sdt + \\sigma(t,s)\\sigma_i s^{\\beta} dW$}, with local volatility overlay
   *  {@latex.inline $\\sigma(t,s)$} and level  {@latex.inline $\\sigma_i $} which takes on one of two values depending on the state of the continuous 2-state hidden Markov chain. The 
   *  resulting pair of PDEs is {@latex.inline $\\frac{\\partial V_1}{\\partial \\tau} - \\sigma(t,s)\\frac{\\sigma_1^2 s^{2\\beta}}{2} \\frac{\\partial^2 V_1}{\\partial s^2} 
   *  -\\mu(t)s \\frac{\\partial V_1}{\\partial s} + (\\mu(t) + \\lambda_{12})V_1 - \\lambda_{12} V_2= 0 \\\\
   ...
   * + (\\mu(t) + \\lambda_{21})V_2 - \\lambda_{21} V_1= 0 $} <p>
   *  As usual with the backwards PDEs, 'time' is time-to-maturity ({@latex.inline $\\tau$}) and thus starts at zero (i.e. at maturity). The initial condition is a call payoff (so 
   *  {@latex.inline $V_1(0,s) = V_2(0,s) = (s-k)^+$}). Real (i.e. calendar) time, t, appears in the local volatility and drift terms with {@latex.inline $t = T - \\tau$} where T
   *  is the maturity. The solution at  {@latex.inline $t = 0 (\\tau=T)$} for   {@latex.inline $V_1$ and $V_2$} is the option price if the Markov chain started in state 1 or 2. If the 
   *  initial state of the Markov chain is unknown (but the probability of being in a state is know), then the option price is the probability weighting of the two solutions. 
   * @param forward The forward curve 
   * @param strike The strike 
  ...
  /**
   *  Set up a forward coupled PDE for a model where the underlying follows the SDE {@latex.inline $ ds = \\mu(t)sdt + \\sigma_i s^{\\beta} dW$}, with  
   *  level  {@latex.inline $\\sigma_i $} which takes on one of two values depending on the state of the continuous 2-state hidden Markov chain,
   *  and the payoff is a standard European call. The coupled PDEs are in maturity (T) and strike (k) coordinates and read
   *   {@latex.inline $\\frac{\\partial V_1}{\\partial T}  - \\frac{\\sigma_1^2 k^{2\\beta}}{2} 
   ...
   *  The initial condition is  {@latex.inline $V_1(0,k) = p_1(0)(s_0-k)^+$} and {@latex.inline $V_2(0,k) = p_2(0)(s_0-k)^+$} where  {@latex.inline $p_1(0)$ and $p_2(0)$} are the probabilities
   *  of the Markov chain starting in a given state and {@latex.inline $s_0$} is the spot. The option value at a particular maturity (T) and strike (k) is given by 
   *   {@latex.inline $V(T,k) = V_1(T,k) + V_2(T,k)$} 
   * @param forward  The forward curve 
   * @param data Data relating to the Markov chain
   * @return a pair of convection diffusion data bundles
   */
  ...
  /**
   *  Set up a forward coupled PDE for a model where the underlying follows the SDE {@latex.inline $ ds = \\mu(t)sdt + \\sigma(t,s)\\sigma_i s^{\\beta} dW$}, with local volatility overlay
   *  {@latex.inline $\\sigma(t,s)$} and level  {@latex.inline $\\sigma_i $} which takes on one of two values depending on the state of the continuous 2-state hidden Markov chain,
   *  and the payoff is a standard European call. The coupled PDEs are in maturity (T) and strike (k) coordinates and read
   *   {@latex.inline $\\frac{\\partial V_1}{\\partial T}  - \\sigma(T,k)\\frac{\\sigma_1^2 k^{2\\beta}}{2} 
   *   \\frac{\\partial^2 V_1}{\\partial k^2} -\\mu(T)k \\frac{\\partial V_1}{\\partial k} + \\lambda_{12}V_1 - \\lambda_{21} V_2= 0 \\\\
  ...
  /**
   * Set up a coupled Fokker-Plank PDE for a model where the underlying follows the SDE {@latex.inline $ ds = \\mu(t)sdt + \\sigma_i s^{\\beta} dW$}, with  level 
   *  {@latex.inline $\\sigma_i $} which takes on one of two values depending on the state of the continuous 2-state hidden Markov chain. The resultant PDE is 
   *  {@latex.inline $\\frac{\\partial \\rho_1}{\\partial t}  -  \\frac{\\partial^2 }{\\partial s^2}\\left[\\frac{\\sigma_1^2 s^{2\\beta}}{2}\\rho_1 \\right] -
   ...
   *  \\frac{\\partial }{\\partial s}\\left[\\mu(t)s\\rho_2\\right] + \\lambda_{21}\\rho_2 - \\lambda_{12} \\rho_1= 0 $} where {@latex.inline $\\rho_i(t,s)$} is the probabilty density 
   *  of being in state i with a vlaue of the underlying s at time t. The inital state is  {@latex.inline $\\rho_i(0,s) = p_i(0)\\delta(s-s_0)$} where {@latex.inline $p_i(0)$} 
   *  is the probability of the Markov chain starting in state i and  {@latex.inline $s_0$} is the spot. Since a delta function cannot be used directly it is replaced with a
   *  log-normal distrubution ($@latex.inline $of s/s_0$}) with a standard deviation of 0.01;
   * @param forward The forward curve 
   * @param data Data relating to the Markov chain
  ...
  /**
   * Set up a coupled Fokker-Plank PDE for a model where the underlying follows the SDE {@latex.inline $ ds = \\mu(t)sdt + \\sigma(t,s)\\sigma_i s^{\\beta} dW$}, with local volatility overlay
   *  {@latex.inline $\\sigma(t,s)$} and level  {@latex.inline $\\sigma_i $} which takes on one of two values depending on the state of the continuous 2-state hidden Markov chain. 
   *  The resultant PDE is 
   *  {@latex.inline $\\frac{\\partial \\rho_1}{\\partial t}  -  \\frac{\\partial^2 }{\\partial s^2}\\left[ \\sigma(t,s)\\frac{\\sigma_1^2 s^{2\\beta}}{2}\\rho_1 \\right] -
   *  \\frac{\\partial}{\\partial s}\\left[\\mu(t)s\\rho_1\\right] + \\lambda_{12}\\rho_1 - \\lambda_{21}\\rho_2= 0 \\\\
