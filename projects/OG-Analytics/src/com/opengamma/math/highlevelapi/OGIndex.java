...
public class OGIndex {
  ...
  public OGIndex getFullRow(int index) {
    int[] tmp = new int[_cols];
    for (int i = 0; i < _cols; i++) {
      tmp[i] = _data[_rowPtr[index] + i];
    }
    int[][] tmp2 = {tmp };
    return new OGIndex(tmp2);
  }

  public OGIndex getFullColumn(int index) {
  ...
  public OGIndex getRowElements(int index) {
    return this.getFullRow(index);
  }

  public OGIndex getColumnElements(int index) {
