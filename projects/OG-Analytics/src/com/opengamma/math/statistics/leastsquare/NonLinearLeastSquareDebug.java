...
//    return solve(observedValues, new DoubleMatrix1D(sigma), func, jac.differentiate(func), startPos, null);
//  }
//
//  /**
//   *  Use this when the model is given as a function of its parameters only (i.e. a function that takes a set of parameters and return a set of model values,
...
//        double temp = deltaTheta.getEntry(i);
//        if (Math.abs(temp) > maxJumps.getEntry(i)) {
//          deltaTheta.getData()[i] = temp > 0 ? maxJumps.getEntry(i) : -maxJumps.getEntry(i);
//        }
//      }
//
...
//        final DoubleMatrix2D alphaTry = getModifiedCurvatureMatrix(jacobian, 0.0);
//        SVDecompositionResult svdRes = svd.evaluate(alphaTry);
...
//        Function1D<DoubleMatrix1D, DoubleMatrix2D[]> secDivFunc = diff.differentiateFull(func);
...
//        }
//
//        DoubleMatrix2D newAlpha = (DoubleMatrix2D) _algebra.add(alphaTry, new DoubleMatrix2D(temp));
//
//        svdRes = svd.evaluate(newAlpha);
//        w = svdRes.getSingularValues();
//        u = svdRes.getU();
//        v = svdRes.getV();
//
//
...
//        if (saddle) {
//          lambda = increaseLambda(lambda);
//          double scale = 0.5 * Math.sqrt(-oldChiSqr / smallest);
//          for (int j = 0; j < nParms; j++) {
//            p[j] = u.getEntry(j,index);
//          }
...
//            i++;
//            if (i > 20) {
//              throw new MathException("Could not move away from maximum/saddle point");
//            }
//          }
//        } else {
