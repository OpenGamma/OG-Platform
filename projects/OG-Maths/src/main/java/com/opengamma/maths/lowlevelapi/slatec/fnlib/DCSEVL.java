...
public class DCSEVL {
  ...
  public static double getDCSEVL(double x, double[] cs, int n) {
    ...
    if (n < 1) {
      throw new MathsExceptionIllegalArgument("DCSEVL: number of terms < 0");
    }
    if (n > 1000) {
      throw new MathsExceptionIllegalArgument("DCSEVL: number of terms > 1000");
    }
    if (Math.abs(x) > s_onepl) {
      throw new MathsExceptionIllegalArgument("DCSEVL: x outside of the interval [-1,+1)");
    }
    if (n > cs.length) {
      throw new MathsExceptionIllegalArgument("DCSEVL: number of terms to compute greater than number of coefficients given (n>cs.length)");
    }  
