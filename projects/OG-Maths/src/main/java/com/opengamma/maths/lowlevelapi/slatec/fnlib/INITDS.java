...
public class INITDS {
  ...
  public static int getInitds(double[] os, int nos, double eta) {
    if (os == null) {
      throw new MathsExceptionIllegalArgument("INITDS: os is null");
    }
    if (nos != os.length) {
      throw new MathsExceptionIllegalArgument("INITDS: nos and os.length are not equal, they should be!");
    }
    if (nos < 1) {
      throw new MathsExceptionIllegalArgument("INITDS: Number of coeffs is less than 1");
    }
    double err = 0;
    int i = 0;
    boolean error = true;
    ...
    if (error) {
      throw new MathsExceptionIllegalArgument("INITDS: Chebyshev series too short for specified accuracy");
    }
    return i;
  }
