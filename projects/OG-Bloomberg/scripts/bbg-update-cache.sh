...
JAVA_OPTS="-Xmx512m -XX:-UseGCOverheadLimit -Dlogback.configurationFile=$BASE_DIR/config/bbgserver-logback.xml"

#Assume we get run once per day, since that's the scope of BBG quotas
SECONDS=`date +%s`
UPDATE_ID=`echo "$SECONDS/86400" | bc` #Days since epoch

$JAVA_HOME/bin/java $JAVA_OPTS -classpath $CLASSPATH com.opengamma.bbg.referencedata.cache.MongoDBReferenceDataCacheRefresher $1 $UPDATE_ID;