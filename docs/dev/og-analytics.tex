\chapter{Introduction}

\chapter{Instruments}

This chapter introduces financial instruments, which implement the << f.javadoc_href("com.opengamma.analytics.financial.interestrate.InstrumentDerivative") >> interface.

<% for class_name in sorted(d['/jsondocs/javadoc-data.json|javadocs']['packages']['com.opengamma.analytics.financial.interestrate']['classes']['InstrumentDerivative'].implementers) -%>
\hyperref[sec:<< class_name >>]{<< class_name >>}

<% endfor %>

\section{Cash}
<% set class_name = "com.opengamma.analytics.financial.interestrate.cash.derivative.Cash" -%>
\label{sec:<< class_name >>}
<% set package_name, unqualified_class_name = f.split_package_class(class_name) -%>

<< f.javadoc_href(class_name, 'javadoc') >>

<< d['/jsondocs/javadoc-data.json|javadocs']['packages'][package_name]['classes'][unqualified_class_name]['comment-text'] >>

