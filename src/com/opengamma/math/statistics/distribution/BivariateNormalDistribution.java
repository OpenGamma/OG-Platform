...
public class BivariateNormalDistribution implements ProbabilityDistribution<Double[]> {
  ...
  private final double[][] _xx = new double[][] { new double[] { -0.932469514203152, -0.981560634246719, -0.9931285991850950 },
  ...
  private final double[][] _w = new double[][] { new double[] { 0.171324492379170, 0.0471753363865118, 0.0176140071391521 },
  ...
  public double getCDF(final Double[] args) {
    final double x = -args[0];
    double y = -args[1];
    final double rho = args[2];
    double xy = x * y;
    double bvn = 0;
    int ng = 2;
    int lg = 9;
    ...
    if (Math.abs(rho) < 0.925) {
      if (Math.abs(rho) > 0) {
        final double xySquared = (x * x + y * y) / 2.;
        final double asinRho = Math.asin(rho);
        for (int i = 0; i <= lg; i++) {
      ...
      if (Math.abs(rho) < 1) {
        final double aSquared = 1 - rho * rho;
        double a = Math.sqrt(aSquared);
        final double bSquared = (x - y) * (x - y);
        final double c = (4 - xy) / 8.;
        final double d = (12 - xy) / 16.;
        final double e = -(bSquared / aSquared + xy) / 2.;
        if (e > -100) {
          bvn = a * Math.exp(e) * (1 - c * (bSquared - aSquared) * (1 - d * bSquared / 5.) / 3. + c * d * aSquared * aSquared / 5.);
        }
        if (-xy < 100) {
          final double b = Math.sqrt(bSquared);
  ...
  private double getCorrection(final double a, final int multiplier, final int row, final int column, final double xy, final double xySquared) {
    final double sin = Math.sin(a * (multiplier * _xx[row][column] + 1) / 2.);
    return _w[row][column] * Math.exp((sin * xy - xySquared) / (1 - sin * sin));
  }

  private double getCorrection(final double a, final double b, final double c, final double d, final int multiplier, final int row, final int column, final double xy) {
    final double xs = Math.pow(a * (multiplier * _xx[row][column] + 1), 2);
    final double rs = Math.sqrt(1 - xs);
    final double e = -(b / xs + xy) / 2.;
    if (e > -100)
