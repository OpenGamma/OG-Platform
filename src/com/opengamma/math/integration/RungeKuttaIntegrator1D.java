 ...
 * specialised methods.
 */
public class RungeKuttaIntegrator1D extends
   ...
   */
	public RungeKuttaIntegrator1D(final double atol, final double rtol,
		...
		if (rtol < 0.0)
			throw new IllegalArgumentException(
					"Relative Tolerance must be grater than zero");
		if (minSteps < 1)
	...
	private double calculateRungeKuttaFourthOrder(
			final Function1D<Double, Double> f, final double x, final double h,
			final double fl, final double fm, final double fu) {
		...
		return calculateRungeKuttaFourthOrder(f, x, h / 2.0, fl, f1, fm)
				+ calculateRungeKuttaFourthOrder(f, x + h / 2.0, h / 2.0, fm,
						f2, fu);
	}
