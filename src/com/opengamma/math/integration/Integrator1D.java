...
public abstract class Integrator1D<T, U, V> implements Integrator<T, Function1D<V, V>, V> {
  ...
  public T integrate(final U f, final V[] lower, final V[] upper) {
    ...
    if (lower.length > 1) {
      s_Log.info("Lower bound array had more than one element; only using the first");
    }
    if (lower[0] == null)
      throw new IllegalArgumentException("Lower bound was null");
    if (upper.length > 1) {
      s_Log.info("Upper bound array had more than one element; only using the first");
    }
    if (upper[0] == null)
