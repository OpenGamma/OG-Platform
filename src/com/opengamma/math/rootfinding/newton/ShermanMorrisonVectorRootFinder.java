...
public class ShermanMorrisonVectorRootFinder extends NewtonVectorRootFinder {
  ...
  public ShermanMorrisonVectorRootFinder(final double absoluteTol, final double relativeTol, final int maxSteps, JacobianCalculator calculator) {
    this(absoluteTol, relativeTol, maxSteps, calculator, new LUDecompositionCommons());
  }

  public ShermanMorrisonVectorRootFinder(final double absoluteTol, final double relativeTol, final int maxSteps, JacobianCalculator calculator,
      MatrixAlgebra algebra) {
    this(absoluteTol, relativeTol, maxSteps, calculator, new LUDecompositionCommons(), algebra);
  }

  public ShermanMorrisonVectorRootFinder(final double absoluteTol, final double relativeTol, final int maxSteps, JacobianCalculator calculator,
      final Decomposition<?> decomp) {
    this(absoluteTol, relativeTol, maxSteps, calculator, decomp, new OGMatrixAlgebra());
  }

  public ShermanMorrisonVectorRootFinder(final double absoluteTol, final double relativeTol, final int maxSteps, final Decomposition<?> decomp,
      MatrixAlgebra algebra) {
    this(absoluteTol, relativeTol, maxSteps, new FiniteDifferenceJacobianCalculator(), decomp, algebra);
  }

  public ShermanMorrisonVectorRootFinder(final double absoluteTol, final double relativeTol, final int maxSteps, JacobianCalculator calculator,
      final Decomposition<?> decomp, MatrixAlgebra algebra) {
    super(absoluteTol, relativeTol, maxSteps, new InverseJacobianDirectionFunction(algebra), new InverseJacobianEstimateInitializationFunction(decomp,
        calculator), new ShermanMorrisonMatrixUpdateFunction(algebra));
  }
