...
public abstract class AnalyticOptionModelFunction extends AbstractFunction implements FunctionInvoker {
  ...
  public Set<ComputedValue> execute(final FunctionExecutionContext executionContext, final FunctionInputs inputs, final ComputationTarget target,
    ...
    final StandardOptionDataBundle data = getDataBundle(option, inputs);
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target) {
    ...
    for (final String valueName : AVAILABLE_GREEKS.keySet()) {
      results.add(new ValueSpecification(new ValueRequirement(valueName, ComputationTargetType.SECURITY, security.getIdentityKey())));
    }
    return results;
  }
  ...
  protected abstract StandardOptionDataBundle getDataBundle(OptionSecurity option, FunctionInputs inputs);
}
