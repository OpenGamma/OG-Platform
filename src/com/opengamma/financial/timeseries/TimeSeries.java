...
public interface TimeSeries<T> extends Iterable<Map.Entry<InstantProvider, T>> {
  /**
   * Get the size of the time series, the number of data points. This is long so
   * that we don't have to recode if we need >2GB timeseries.
   * 
   * @return the size
   */
  public int size();
   ...
   * duration. If the start or end time are not present in the series then a
   * NoSuchElementException will be thrown.
   * 
   * @throws NoSuchElementException
   * @param start
