...
public class AutoregressiveModel implements TimeSeriesModel {
  // TODO change date to jsr-310 date
  ...
  public List<Double> getSeries(int n, double mean, List<Double> phi, int num) {
    if(phi.size() != n) throw new IllegalArgumentException("Number of coefficients must equal the order of the model: have " + phi.size() + " coefficients for an AR(" + n + ") model");
