...
public abstract class RecombiningTree<T> implements Lattice<T> {
  ...
  public T getNode(int step, int node) {
    if (step > _tree.length) throw new IllegalArgumentException("Step number " + step + " is greater than maximum in this tree (max =  " + _tree.length + ")");
    int max = getMaxNodesForStep(step);
    if (node > max) throw new IllegalArgumentException("Node number " + node + " is greater than the number of nodes at this step number (max = " + max + ")");
