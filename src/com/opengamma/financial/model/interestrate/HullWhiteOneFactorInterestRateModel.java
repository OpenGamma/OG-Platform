...
public class HullWhiteOneFactorInterestRateModel {
  ...
  public Function1D<HullWhiteOneFactorInterestRateDataBundle, Double> getInterestRateFunction(final ZonedDateTime time,
    ...
    return new Function1D<HullWhiteOneFactorInterestRateDataBundle, Double>() {
      ...
      public Double evaluate(final HullWhiteOneFactorInterestRateDataBundle data) {
        ...
        final double lnA = Math.log(ps / pT) - b * dlnPdt - sigma * sigma
            * Math.pow(Math.exp(-speed * s) - Math.exp(-speed * t), 2) * (Math.exp(2 * speed * t) - 1)
            / (4 * speed * speed * speed);
