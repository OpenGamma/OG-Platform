...
public class PortfolioTreeNode extends DirectBean implements MutableUniqueIdentifiable {
  ...
  // AIWG: The methods below were to get tests to run; don't merge them back in - the Joda bean impl should be Fudge aware by now

  public FudgeFieldContainer toFudgeMsg(final FudgeSerializationContext context) {
    final MutableFudgeFieldContainer message = context.newMessage();
    context.objectToFudgeMsg(message, "uniqueIdentifier", null, _uniqueIdentifier);
    context.objectToFudgeMsg(message, "name", null, _name);
    context.objectToFudgeMsg(message, "childNodes", null, _childNodes);
    return message;
  }

  @SuppressWarnings("unchecked")
  public static PortfolioTreeNode fromFudgeMsg(final FudgeDeserializationContext context, final FudgeFieldContainer message) {
    final PortfolioTreeNode node = new PortfolioTreeNode();
    FudgeField field = message.getByName("uniqueIdentifier");
    if (field != null) {
      node.setUniqueIdentifier(context.fieldValueToObject(UniqueIdentifier.class, field));
    }
    field = message.getByName("name");
    if (field != null) {
      node.setName(message.getFieldValue(String.class, field));
    }
    field = message.getByName("childNodes");
    if (field != null) {
      node._childNodes = context.fieldValueToObject(List.class, field);
    }
    return node;
  }
